...
template <class GraphEdgePtr, class GraphNodePtr> class GEIteratorWorklist;

// The iteration proceeds from the starting nodes along incoming or outgoing
// edges. When new nodes are added to the iterator they can be placed into the
// worklist according to several policies:
typedef enum {
  succ_front, // The new node is visited next.
  succ_back,  // The new node is visited after all the nodes currently on the
              // worlist
  topo_order, // All graph nodes are ordered topologically and all nodes on the
              // worklist
...
template <class GraphEdgePtr, class GraphNodePtr> class graphEdgeIterator {
public:
  // Indicates whether the graph is being created incrementally as it is being
  // traversed (true) or whether the graph has already been created (false). If
  // the former, it is expected that nodes will implement method
  // getSupersetPart() that returns the node that they are refining in a graph
  // that is guaranteed to have already been created, and same for edges (method
  // getSupersetPartEdge()).
...
public:
  // Records whether this iterator has been initialized with some starting
  // node(s)
  bool initialized;
...
public:
  graphEdgeIterator(bool incrementalGraph, iterDirection dir,
                    graphIterOrderT iterOrder = succ_back,
                    bool initialized = false);
  graphEdgeIterator(const GraphNodePtr start, bool incrementalGraph,
                    iterDirection dir, graphIterOrderT iterOrder = succ_back);
  graphEdgeIterator(const std::set<GraphNodePtr> &start, bool incrementalGraph,
                    iterDirection dir, graphIterOrderT iterOrder = succ_back);
  graphEdgeIterator(const graphEdgeIterator &that);
  virtual ~graphEdgeIterator() {}

  // Creates a new worklist implementation that follows the given iteration
  // order and returns a pointer to it.
  boost::shared_ptr<GEIteratorWorklist<GraphEdgePtr, GraphNodePtr>>
  createWorklist(graphIterOrderT iterOrder);

  void init(const GraphNodePtr start);
  void init(const std::set<GraphNodePtr> &start);
  void init(const std::set<GraphEdgePtr> &start);

  // Given a part returns the wildcard edge pointing into it (for fw traversal)
  // or out of it (for backward traversals
...
protected:
  // Returns true if the given GraphNodePtr is in the worklist and false
  // otherwise
  // bool isRemaining(const GraphEdgePtr pedge);

  // Find the followers of curPart (either successors or predecessors, depending
  // on value of fwDir), push back those that have not yet been visited
...
public:
  ...
  // Grab the PartEdge that the iterator is currently is referring to and
  // advance it to the following PartEdge
  ...
  public:
    checkpoint(boost::shared_ptr<GEIteratorWorklist<GraphEdgePtr, GraphNodePtr>>
                   worklist,
               const std::set<GraphEdgePtr> &visited);
...
class fw_graphEdgeIterator
    : public virtual graphEdgeIterator<GraphEdgePtr, GraphNodePtr> {
public:
  fw_graphEdgeIterator(bool incrementalGraph,
                       graphIterOrderT iterOrder = succ_back,
                       bool initialized = false);
  fw_graphEdgeIterator(const GraphNodePtr start, bool incrementalGraph,
                       graphIterOrderT iterOrder = succ_back);
...
template <class GraphEdgePtr, class GraphNodePtr>
class bw_graphEdgeIterator
    : public virtual graphEdgeIterator<GraphEdgePtr, GraphNodePtr> {
public:
  bw_graphEdgeIterator(bool incrementalGraph,
                       graphIterOrderT iterOrder = succ_back,
                       bool initialized = false);
  bw_graphEdgeIterator(const GraphNodePtr start, bool incrementalGraph,
                       graphIterOrderT iterOrder = succ_back);
...
template <class GraphEdgePtr, class GraphNodePtr>
class dataflowGraphEdgeIterator
    : public virtual graphEdgeIterator<GraphEdgePtr, GraphNodePtr> {
protected:
...
public:
  ...
  dataflowGraphEdgeIterator(
      bool incrementalGraph,
      typename graphEdgeIterator<GraphEdgePtr, GraphNodePtr>::iterDirection dir,
      graphIterOrderT iterOrder);
  dataflowGraphEdgeIterator(
  ...
  void init(const GraphNodePtr &start_arg,
            const std::set<GraphNodePtr> &terminators_arg);
  void init(const std::set<GraphNodePtr> &start_arg,
            const std::set<GraphNodePtr> &terminators_arg);
  ...
  public:
    checkpoint(
        const typename graphEdgeIterator<GraphEdgePtr, GraphNodePtr>::checkpoint
            &iChkpt,
        const std::set<GraphNodePtr> &terminators);
...
typedef dataflowGraphEdgeIterator<PartEdgePtr, PartPtr>
    dataflowPartEdgeIterator;

template <class GraphEdgePtr, class GraphNodePtr>
class fw_dataflowGraphEdgeIterator
    : public virtual dataflowGraphEdgeIterator<GraphEdgePtr, GraphNodePtr>,
...
typedef fw_dataflowGraphEdgeIterator<PartEdgePtr, PartPtr>
    fw_dataflowPartEdgeIterator;

template <class GraphEdgePtr, class GraphNodePtr>
class bw_dataflowGraphEdgeIterator
    : public virtual dataflowGraphEdgeIterator<GraphEdgePtr, GraphNodePtr>,
...
typedef bw_dataflowGraphEdgeIterator<PartEdgePtr, PartPtr>
    bw_dataflowPartEdgeIterator;
...
current dominator set must be
      ...
      if(dominators.find(target) == dominators.end()) {
        // The intersection of all parts with dominators[part] is
dominators[part] dominators[target] = dominators[part]; } else {
...
dominators[part].begin(), dominators[part].end(), .begin());
        ...
        while(partIt!=dominators[part].end() &&
descIt!=dominators[target].end()) { if(*partIt < *descIt) partIt++; else
if(*partId > *descIt) descIt = dominators[target].erase(descIt); else if(*partId
== *descIt) { partIt++; descIt++;
          }
        }
...
// Base class for various implementations of worklist data structures and
// iteration order. There is class that derives from GEIteratorWorklist for each
// value of enum graphIterOrderT. Implementations of iteration worklists are
// allowed to keep multiple copies of an edge in the worklist at the same time
// or may optionally eliminate duplicates.
template <class GraphEdgePtr, class GraphNodePtr> class GEIteratorWorklist {
protected:
  // public:
  //  The iterator that this worklist is being used from. The worklist may use
  //  the iterator for things like accessing the successors of a given graph
  //  node
  graphEdgeIterator<GraphEdgePtr, GraphNodePtr> *parent;

public:
  GEIteratorWorklist(graphEdgeIterator<GraphEdgePtr, GraphNodePtr> *parent)
      : parent(parent) {}

  GEIteratorWorklist(const GEIteratorWorklist &that) : parent(that.parent) {}

  // Returns a freshly-allocated copy of the worklist
  virtual boost::shared_ptr<GEIteratorWorklist<GraphEdgePtr, GraphNodePtr>>
  copy() const = 0;
  ...
  virtual bool
  eq(const GEIteratorWorklist<GraphEdgePtr, GraphNodePtr> &that) const = 0;
  virtual bool
  eq(boost::shared_ptr<GEIteratorWorklist<GraphEdgePtr, GraphNodePtr>> that) {
    return eq(*that.get());
  }
...
// Implementation of the graph iterator worklist that places all new edges
// before or after all of the edges currently in the worklist. Edges are always
// grabbed from the front of the list.
template <class GraphEdgePtr, class GraphNodePtr>
class GEFrontBackIteratorWorklist
    : public GEIteratorWorklist<GraphEdgePtr, GraphNodePtr> {
protected:
  std::list<GraphEdgePtr> worklist;
...
class GETopoOrderIteratorWorklist
    : public GEIteratorWorklist<GraphEdgePtr, GraphNodePtr> {
protected:
  // Map from each edge's unique topological index to its object. If the graph
  // is incremental these are edges in the superset graph that is already
  // created. Otherwise, they are edges in the graph being traversed. This data
  // structure makes it possible to efficiently find the edge with the smallest
  // index.
  std::map<int, std::set<GraphEdgePtr>> worklist;

  // Maps each edge and node to its topological index to efficiently figure out
  // where into the worklist to place newly-observed edges. If the graph is
  // incremental these are edges in the superset graph that is already created.
...
public:
  ...
  GETopoOrderIteratorWorklist(const GETopoOrderIteratorWorklist &that)
      : GEIteratorWorklist<GraphEdgePtr, GraphNodePtr>(that),
        worklist(that.worklist), edge2Idx(that.edge2Idx),
        node2Idx(that.node2Idx), maxEdgeTopoIdx(that.maxEdgeTopoIdx),
