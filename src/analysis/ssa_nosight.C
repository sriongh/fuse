// This file holds the portions of partitions.C that cannot be compiled when
// sight.h is included
...
const SSAGraph::VertexVertexMap &SSAGraph::getDominatorTree() // const
{
  ...
  // Here we use the algorithm in boost::graph to build a map from each node to
  // its immediate dominator.
  StartPartPtr start = makePtr<StartPart>(startStates);
  boost::lengauer_tarjan_dominator_tree(*this, nodesToVertices[start],
                                        domTreePredMap);
  return dominatorTree;
}
...
const SSAGraph::VertexVertexMap &SSAGraph::getPostdominatorTree() // const
{
  ...
  boost::associative_property_map<VertexVertexMap> postdomTreePredMap(
      postdominatorTree);

  std::set<PartPtr> endStates = comp->GetEndAStates(analysis);

  // Here we use the algorithm in boost::graph to build an map from each node to
  // its immediate dominator.
  ...
  boost::lengauer_tarjan_dominator_tree(boost::make_reverse_graph(*this),
                                        nodesToVertices[end],
                                        postdomTreePredMap);
  return postdominatorTree;
}
...
void SSAGraph::calculateDominanceFrontiers() {
  // cout << "SSAGraph::calculateDominanceFrontiers()
  // #dominanceFrontiers="<<dominanceFrontiers.size()<<endl;
  ...
  // TODO: This code converts a VertexVertex Map to a  boost graph. Should be
  // factored out
  ...
  BOOST_FOREACH (SSAGraph::VertexVertexMap::value_type &nodeDominatorPair,
                 dominatorTreeMap) {
  ...
  // Calculate all the dominance frontiers. This algorithm is from figure 10,
  // Cytron et. al 1991
  ...
  /*  cout << "    nodesToVertices(#"<<nodesToVertices.size()<<")="<<endl;
    for(map<PartPtr, Vertex>::iterator i=nodesToVertices.begin();
    i!=nodesToVertices.end(); i++) cout << "        "<<i->second<<":
    "<<i->first->str()<<endl;*/

  BOOST_FOREACH (TreeVertex v, reverseTopological) {
    PartPtr currentNode = domTree[v];
    // cout << "currentNode="<<currentNode->str()<<",
    // #dominanceFrontiers="<<dominanceFrontiers.size()<<endl;
    set<PartPtr> &currentDominanceFrontier = dominanceFrontiers[currentNode];

    // Local contribution: Iterate over all the successors of v in the control
    // flow graph
    ...
    BOOST_FOREACH (PartEdgePtr outEdge, currentNode->outEdges()) {
...
#if !USE_ROSE
      // DQ (11/3/2011): EDG compilains about this (but GNU allowed it, I think
      // that EDG might be correct. since it might be a private variable.  But
      // since we are only trying to compile ROSE with ROSE (using the new
      // EDG 4.3 front-end as a tests) we can just skip this case for now.
      ...
      if (dominatorTreeMap.count(successorVertex) == 1) {
        ...
        // If we have a successor that we don't dominate, that successor is in
        // our dominance frontier
    ...
    for (tie(currentIter, lastIter) = adjacent_vertices(v, domTree);
         currentIter != lastIter; currentIter++) {
      PartPtr childNode = domTree[*currentIter];
      const set<PartPtr> &childDominanceFrontier =
          dominanceFrontiers[childNode];
...
#if !USE_ROSE
        // DQ (11/3/2011): EDG compilains about this (but GNU allowed it, I
        // think that EDG might be correct. since it might be a private
        // variable.  But since we are only trying to compile ROSE with ROSE
        // (using the new EDG 4.3 front-end as a tests) we can just skip this
        // case for now.
  ...
  BOOST_FOREACH (SSAGraph::VertexVertexMap::value_type &nodePostDominatorPair,
                 postDominatorTreeMap) {
