...
namespace fuse {
// Wrapper for boost:shared_ptr<Type> that can be used as keys in maps because
// it wraps comparison operations by forwarding them to the Type's own
// comparison operations. In contrast, the base boost::shared_ptr uses pointer
// equality.
template <class Type>
...
public:
  ...
  // copy - If true, the shared pointer will refer to a copy of the original
  // object
  CompSharedPtr(Type *p, bool copy = false) {
    Type *c;
    if (copy)
      c = dynamic_cast<Type *>(p->copy());
    else
      c = p;
    assert(c);
  ...
  // Copy constructor
  CompSharedPtr(const CompSharedPtr<Type> &o)
      : boost::shared_ptr<Type>(o) {} // ptr(o.ptr) {}

  // Constructor for converting across CompSharedPtr wrappers of compatible
  // types
  template <class OtherType>
  CompSharedPtr(const CompSharedPtr<OtherType> &o)
      : boost::shared_ptr<Type>(boost::static_pointer_cast<Type>(o)) {
  } // ptr(boost::static_pointer_cast<Type>(o.ptr)) {}

  // Constructor for wrapping boost::shared_ptr with a CompoSharedPtr
  CompSharedPtr(boost::shared_ptr<Type> ptr)
      : boost::shared_ptr<Type>(ptr) {} // ptr(ptr) {}

  template <class OtherType>
  CompSharedPtr(boost::shared_ptr<OtherType> ptr)
      : boost::shared_ptr<Type>(boost::static_pointer_cast<Type>(ptr)) {
  } // ptr(boost::static_pointer_cast<Type>(ptr)) {}

  template <class OtherType> friend class CompSharedPtr;

  // CompSharedPtr& operator=(const CompSharedPtr& o) { ptr = o.ptr; return
  // *this; }
  //  If both ptr and o.ptr are != NULL, use their equality operator
  //  If both ptr and o.ptr are == NULL, they are equal
  //  If only one is == NULL but the other is not, order the NULL object as <
  //  the non-NULL object
  bool operator==(const CompSharedPtr<Type> &o) const {
    if (/*ptr.*/ this->get() != NULL) {
      if (o.get() != NULL) { /*dbg << "CompSharedPtr == returning
                                "<<((*ptr.get()) == o)<<std::endl;*/
        return (* /*ptr.*/ this->get()) == o;
      } else
        return false;
    } else {
      if (o.get() != NULL)
  ...
  std::string str(std::string indent = "") const {
    return /*ptr->*/ this->get()->str(indent);
  }
};

// Returns a new instance of a CompSharedPtr that refers to an instance of
// CompSharedPtr<Type> GB 2012-09-21: We have created an instance of this
// function for cases with 0-9 input parameters since that is
//                what boost::make_shared provides when the compiler doesn't
//                support varyadic types. Support for varyadic types is future
//                work.
template <class Type> CompSharedPtr<Type> makePtr() {
...
CompSharedPtr<Type>
makePtr(const Arg1 &arg1, const Arg2 &arg2, const Arg3 &arg3, const Arg4 &arg4,
...
makePtr(const Arg1 &arg1, const Arg2 &arg2, const Arg3 &arg3, const Arg4 &arg4,
        const Arg5 &arg5, const Arg6 &arg6, const Arg7 &arg7, const Arg8 &arg8,
        const Arg9 &arg9, const Arg10 &arg10) {
  return boost::make_shared<Type>(arg1, arg2, arg3, arg4, arg5, arg6, arg7,
                                  arg8, arg9, arg10);
}
...
makePtr(const Arg1 &arg1, const Arg2 &arg2, const Arg3 &arg3, const Arg4 &arg4,
        const Arg5 &arg5, const Arg6 &arg6, const Arg7 &arg7, const Arg8 &arg8,
        const Arg9 &arg9, const Arg10 &arg10, const Arg11 &arg11) {
  return boost::shared_ptr<Type>(new Type(arg1, arg2, arg3, arg4, arg5, arg6,
...
CompSharedPtr<TargetType> dynamicPtrCast(CompSharedPtr<SourceType> s) {
  if (dynamic_cast<TargetType *>(s.get()))
...
template <class TargetType, class SourceType>
const CompSharedPtr<TargetType>
dynamicConstPtrCast(CompSharedPtr<SourceType> s) {
  if (dynamic_cast<const TargetType *>(s.get()))
    return CompSharedPtr<TargetType>(s);
  else {
    CompSharedPtr<TargetType> null;
    return null;
...
CompSharedPtr<TargetType> staticPtrCast(const CompSharedPtr<SourceType> s) {
  if (static_cast<TargetType *>(s.get()))
    return CompSharedPtr<TargetType>(s);
  else {
    CompSharedPtr<TargetType> null;
    return null;
...
template <class TargetType, class SourceType>
const CompSharedPtr<TargetType>
staticConstPtrCast(const CompSharedPtr<SourceType> s) {
  if (static_cast<const TargetType *>(s.get()))
    return CompSharedPtr<TargetType>(s);
  else {
    CompSharedPtr<TargetType> null;
    return null;
...
// copy - If true, the shared pointer will refer to a copy of the original
// object
template <class Type> CompSharedPtr<Type> initPtr(Type *p, bool copy = false) {
