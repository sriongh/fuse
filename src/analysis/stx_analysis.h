...
// available about the memory locations, values and control locations at at CFG
// node. It can precisely interpret static variables and lexical values but
// provides no information that cannot be directly derived from the text of the
// code such as the referents of pointers.
...
public:
  SyntacticAnalysis() { initGlobalDeclarations(); }
  ...
  void runAnalysis(const Function &func, NodeState *state, bool,
                   std::set<Function>) {}

  // The genInitLattice, genInitFact and transfer functions are not implemented
  // since this is not a dataflow analysis.
  ...
  static void addFunctionEntries(std::set<ArgPartPtr> &states,
  ...
  std::string str(std::string indent = "") const { return "SyntacticAnalysis"; }
}; // SyntacticAnalysis
...
class StxPart : public Part {
  ...
  template<class T>
  friend boost::shared_ptr<T> boost::make_shared<T>(CFGNode n, ComposedAnalysis*
  analysis, bool (*f) (CFGNode));*/

public:
  StxPart(CFGNode n, ComposedAnalysis *analysis,
          bool (*f)(CFGNode) = defaultFilter)
      : Part(analysis, NULLPart, StxFuncContext::getContext(n)), n(n),
        filter(f) {}
  StxPart(const StxPart &part)
      : Part((const Part &)part), n(part.n), filter(part.filter) {}
  StxPart(const StxPartPtr &part)
      : Part((const Part &)part), n(part->n), filter(part->filter) {}
  StxPart(const StxPart &part, bool (*f)(CFGNode) = defaultFilter)
      : Part((const Part &)part), n(part.n), filter(f) {}
  StxPart(const StxPartPtr &part, bool (*f)(CFGNode) = defaultFilter)
      : Part((const Part &)part), n(part->n), filter(f) {}
...
public:
  ...
  // a shared pointer to themselves. These methods return a valid StxPartPtr if
  // the CFNode is interesting and a NULLStxPart otherwise.
  static StxPartPtr create(CFGNode n, ComposedAnalysis *analysis,
                           bool (*f)(CFGNode) = defaultFilter);
...
protected:
  StxPartEdge(CFGNode src, CFGNode tgt, ComposedAnalysis *analysis,
              bool (*f)(CFGNode) = defaultFilter)
      : PartEdge(analysis, NULLPartEdge), p(CFGEdge(src, tgt)), filter(f) {}
  StxPartEdge(CFGPath p, ComposedAnalysis *analysis,
              bool (*f)(CFGNode) = defaultFilter)
      : PartEdge(analysis, NULLPartEdge), p(p), filter(f) {}
  StxPartEdge(const StxPartEdge &dfe)
      : PartEdge((const PartEdge &)dfe), p(dfe.p), filter(dfe.filter) {}

public:
  virtual void init();

  // Returns whether an edge from the given source to the given target
  // corresponds to a valid edge in the Fuse portion of the Virtual CFG
  ...
  static StxPartEdgePtr create(CFGNode src, CFGNode tgt,
                               ComposedAnalysis *analysis,
                               bool (*f)(CFGNode) = defaultFilter);
  static StxPartEdgePtr create(CFGPath p, ComposedAnalysis *analysis,
                               bool (*f)(CFGNode) = defaultFilter);
...
// StxValueObject form a set hierarchy and thus implement the
// AbstractObjectHierarchy interface. The hierarchy is: empty key: any value
// (val==NULL) non-empty: specific value val!=NULL The keys the differentiate
// StxValueObjects are: std::list<comparableSgNode>
...
public:
  ...
  // and false otherwise If both SgValueExp denote different types, returns
  // false
...
/*class StxCodeLocObject : public CodeLocObject
{
  ...
  // Returns whether the two abstract objects denote the same set of concrete
objects bool equalSetCL(CodeLocObjectPtr o, PartEdgePtr pedge);
...
may be equal) of the set denoted
  ...
  // Returns true if this object is live at the given part and false otherwise
  // We don't currently support scope analysis for CodeLocObjects, so we default
to them all being live. bool isLiveCL(PartEdgePtr pedge) { return true; }
...
// This implementation is tied to the ROSE AST (another set of base classes such
// as expression, named, and aliased objects)
//
...
public:
  ...
  typedef enum {
    expr,    // Temporary storage of an expression's result
    named,   // Memory of a named variable
    storage, // Heap, stack or global memory that is not identified with a known
             // named variable. The set of all storage memory regions contains
             // every named region but no expression regions.
    all // All memory, including heap, stack, global and expressions. Only used
        // as a result of merging an expression regions and storage/named
        // regions.
  } regType;
  ...
  public:
    ...
    bool equal(const comparable &that_arg) const {
      // try {
      const comparableType &that =
          dynamic_cast<const comparableType &>(that_arg);
  ...
  // Returns a unique ID that differentiates this object from others within its
  // type. Storage and All objects must have a single ID, while others depend on
  // the expression and symbol they are associated with
  ...
  // Appends to the given hierarchical key the additional information that
  // uniquely identifies this type's set
...
// StxMemRegionObjects form a set hierarchy and thus implement the
// AbstractionHierarchy interface. The hierarchy is: empty key: all non-empty:
//   expr
//     SgExpression*
//   named
//     SgInitializedName*
...
// The memory region of the temporary that holds the intermediate result of an
// expression
...
public:
  ...
  // freshly-allocated StxExprMemRegionType that represents it. Otherwise,
  // returns NULL.
...
public:
  StxNamedMemRegionType(SgInitializedName *iname, SgSymbol *symbol)
      : iname(iname), symbol(symbol) {}
  ...
  std::set<SgNode *> getConcrete() {
    std::set<SgNode *> ret;
    ret.insert(isSgVariableSymbol(symbol));
    return ret;
  }
...
public:
  ...
  // If the given SgNode corresponds to an storage memory region, returns a
  // freshly-allocated StxStorageMemRegionType that represents it. Otherwise,
  // returns NULL. It is assumed that n is not a named or expression type. As
  // such, this method always returns a valid object since Unknown types cover
  // all cases not covered by expr or named
  static StxStorageMemRegionTypePtr getInstance(SgNode *n);

  // Returns the type of this object: expr, named, storage or all
  StxMemRegionType::regType getType() const {
    return StxMemRegionType::storage;
  }
  ...
  // Appends to the given hierarchical key the additional information that
  // uniquely identifies this type's set Nothing added since all instances of
  // the storage type denote the same set. However, we record that we've reached
  // the end of the hierarchy since no additional precision can be added
  // hierarchically to objects built from this MemRegion
  void addHierSubKey(const AbstractionHierarchy::hierKeyPtr &key) {
...
// The single memory region that contains all regions in stack+globals+heap as
// well as all expression regions
...
public:
  ...
  // Appends to the given hierarchical key the additional information that
  // uniquely identifies this type's set Nothing added since all instances of
  // the all type denote the same full set
  void addHierSubKey(const AbstractionHierarchy::hierKeyPtr &key) {}
...
// Given a vector for base VirtualCFG Nodes, get the corresponding refined Parts
// from this composer and add them to the given set of refined Parts
...
// Generic wrapper for comparing SgValueExps's that implements the comparable
// interface by looking at the values encoded by the SgValueExps
