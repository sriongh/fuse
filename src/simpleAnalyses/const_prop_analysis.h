...
// CPValueLattice are used to identify both raw values and the locations of
// class fields within a class and the result of various arithmetic operatons
// among them. There may be arbitrary amounts of padding between fields within
// an object, meaning that we can only determine the order of their offsets
// within the object but not the actual concrete values of these offsets. Our
// representation of constants of either type is based on the Concrete ValueKind
// class from concrete_values.h
class CPValueLattice : public FiniteLattice,
                       public boost::enable_shared_from_this<CPValueLattice> {
...
public:
  ...
  // than its parent
  CPValueObjectPtr shared_from_this() {
    return boost::static_pointer_cast<CPValueObject>(
...
public:
  CPMemLocObject(bool isCPFull, bool isCPEmpty, SgNode *base, PartEdgePtr pedge,
                 ConstantPropagationAnalysis *analysis)
      : MemLocObject(base), analysis(analysis), isCPFull(isCPFull),
        isCPEmpty(isCPEmpty) {}

  CPMemLocObject(MemRegionObjectPtr region, CPValueObjectPtr index,
                 SgNode *base, PartEdgePtr pedge,
                 ConstantPropagationAnalysis *analysis)
      : MemLocObject(region, index, base), analysis(analysis), isCPFull(false),
        isCPEmpty(false) {}

  CPMemLocObject(const CPMemLocObject &that)
      : MemLocObject(that), analysis(that.analysis), isCPFull(false),
        isCPEmpty(false) {}
  ...
  /*
    ...
    // Computes the meet of this and that and saves the result in this
    // returns true if this causes this to change and false otherwise
    bool intersectUpdate(CPMemLocObject* that, PartEdgePtr pedge, Composer*
...
public:
  ...
  ComposedAnalysisPtr copy() {
    return boost::make_shared<ConstantPropagationAnalysis>(useSSA);
  }
  ...
  // pretty print for the object
  std::string str(std::string indent = "") const { return "ConstPropAnalysis"; }

  friend class ConstantPropagationAnalysisTransfer;
}; // class ConstantPropagationAnalysis

class ConstantPropagationAnalysisTransfer
    : public VariableStateTransfer<CPValueLattice,
                                   ConstantPropagationAnalysis> {
private:
