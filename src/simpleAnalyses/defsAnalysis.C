...
//     SIGHT_VERB_DECL(scope, (txt()<<"DefsAnalysis::addDef(expr=" <<
//     SgNode2Str(expr) << ")"), 1, defsDebugLevel) SIGHT_VERB(dbg << " ml=" <<
//     ml->str() << endl, 1, defsDebugLevel) MemRegionObjectPtr mr =
//     ml->getRegion(); if (ml->getRegion()->isConcrete()) {
//       set<SgNode*> defSyms = ml->getRegion()->getConcrete();
//       for (set<SgNode*>::iterator d = defSyms.begin(); d != defSyms.end();
//       d++)
//         if (isSgVariableSymbol(*d)) {
//           SIGHT_VERB(dbg << "        Adding def " <<
//           SgNode2Str(isSgVariableSymbol(*d)) << " : " <<
//           vIDMap.variableId(isSgVariableSymbol(*d)).toString() << endl, 1,
//           defsDebugLevel)
...
//     SIGHT_VERB_DECL(scope, (txt() << "DefsAnalysis::operator(" <<
//     CFGNode2Str(n) << ")"), 1, defsDebugLevel) std::set<PartEdgePtr> refined;
//     //collectRefinedEdges(composer, refined, n.outEdges());
//     collectOutgoingRefinedEdges(composer, refined, n);
//     SIGHT_VERB(dbg << "#refined=" << refined.size() << endl, 1,
//     defsDebugLevel)
//
//     for (std::set<PartEdgePtr>::iterator r = refined.begin(); r !=
//     refined.end(); r++) {
//       SIGHT_VERB(dbg << "    edge=" << (*r)->str() << endl, 1,
//       defsDebugLevel) set < MemLocObjectPtr > defs; if
...
//     } else if (isSgFunctionDeclaration(target) ||
//     isSgFunctionDefinition(target)
//         || isSgPragmaDeclaration(target)) {
//     } else if ((isSgExpression(target) && isSgVarRefExp(target))
//         || isSgInitializedName(target)) {
//       SIGHT_VERB_DECL(scope, (txt() << "DefsAnalysis::runAnalysis(" <<
//       SgNode2Str(target) << ")"), 1, defsDebugLevel)
//       /*CFGNode start;
...
//        else if(isSgAddressOfOp(target) || isSgPointerDerefExp(target) ||
//        isSgPlusPlusOp(target) || isSgMinusMinusOp(target)) start =
//        CFGNode(target,  1); else                   start = CFGNode(target,
//        2);
//        }
...
//        std::vector<CFGEdge> startOutIn =
//        startOut.begin()->target().inEdges();
...
////      cout << "start="<<CFGNode2Str(start)<<",
///end="<<CFGNode2Str(end)<<endl; /      mapPath(*this, start, start);
//      StxPartPtr start;
...
//      SIGHT_VERB(dbg << "Initial start=" << start.str() << endl, 1,
//      defsDebugLevel)
//
//      StxPartPtr end = StxPart::create(cfgEndOfConstruct(target), NULL);
//      SIGHT_VERB(dbg << "Initial end=" << end.str() << endl, 1,
//      defsDebugLevel)
//
...
//        ROSE_ASSERT(startOut.size() > 0);
//        SIGHT_VERB(dbg << "    start->out=" << (*startOut.begin())->str() <<
//        endl, 1, defsDebugLevel)
//        /*      std::list<StxPartEdgePtr> startOutIn =
//        (*startOut.begin())->stxTarget()->inStxEdges();
//         ROSE_ASSERT(startOutIn.size()>0);
//         start = (*startOutIn.begin())->source();*/
//        start = (*startOut.begin())->stxTarget();
//        SIGHT_VERB(dbg << "    Final start=" << start.str() << endl, 1,
//        defsDebugLevel)
//      }
//
//      // Set endNodes to be all the siblings of end (successors of end's
//      predecessors) std::set<CFGNode> endNodes; std::list<StxPartEdgePtr>
//      endIn = end->inStxEdges(); ROSE_ASSERT(endIn.size() > 0); for
//      (std::list<StxPartEdgePtr>::iterator in = endIn.begin(); in !=
//      endIn.end(); ++in) {
//        SIGHT_VERB(dbg << "    end->in=" << (*in)->str() << endl, 1,
//        defsDebugLevel) std::list<StxPartEdgePtr> endInOut =
//        (*in)->stxSource()->outStxEdges(); SIGHT_VERB(dbg << "    endInOut(#"
//        << endInOut.size() << ")" << endl, 1, defsDebugLevel)
//        ROSE_ASSERT(endInOut.size() > 0);
//        for (std::list<StxPartEdgePtr>::iterator inout = endInOut.begin();
//        inout != endInOut.end(); ++inout) {
//          SIGHT_VERB(dbg << "        inout=" << (*inout)->str() << endl, 1,
//          defsDebugLevel) StxPartPtr endTarget = (*inout)->target();
//          SIGHT_VERB(dbg << "        endTarget(#" <<
//          endTarget->CFGNodes().size() << ")="<< endTarget->str() << endl, 1,
//          defsDebugLevel) std::set<CFGNode> endTargetNodes =
//          endTarget->CFGNodes(); for (std::set<CFGNode>::iterator cn =
//          endTargetNodes.begin(); cn != endTargetNodes.end(); ++cn) {
//            endNodes.insert(*cn);
//            SIGHT_VERB(dbg << "        endNode=" << CFGNode2Str(*cn) << endl,
//            1, defsDebugLevel)
//          }
//        }
//      }
...
//      dbg << "endNodes=" << endl;
//      for (std::set<CFGNode>::iterator n = endNodes.begin(); n !=
//      endNodes.end(); ++n)
//        dbg << "      " << CFGNode2Str(*n) << endl;
//      SIGHT_VERB_FI()
//      std::set<CFGNode> startNodes = start->CFGNodes();
//      assert(startNodes.size() == 1);
//      //std::set<CFGNode> endNodes   = end->CFGNodes();
//      assert(endNodes.size()==1); mapPath(*this, *startNodes.begin(),
//      endNodes, /*includeEndNode*/
//          isSgInitializedName(target));
//    }
//  }
//
...
//  std::map<Label, VariableIdMapping::VariableIdSet>
//  FuseRDAstAttribute::label2defsMap;
...
//      ROSE_ASSERT(node);
//      SIGHT_VERB_DECL(scope, (txt() << "Placing label at " <<
//      SgNode2Str(node)), 1, defsDebugLevel)
//      //node->setAttribute("fuse_cp_above", new ValueASTAttribute(node,
//      composer, cdip, above, "fuse_cp_above")); node->setAttribute("fuse_rd",
//          new FuseRDAstAttribute(composer, cdip, vIDMap, *i, labeler));
//    }
//  }
//
//
// FuseRDAstAttribute::FuseRDAstAttribute(Fuse& f, Label label, Labeler&
// labeler, VariableIdMapping& varIDMap) : f(f), label(label)
//{
...
//    else if(isSgAddressOfOp(sgn) || isSgPointerDerefExp(sgn) ||
//    isSgPlusPlusOp(sgn) || isSgMinusMinusOp(sgn)) cn = CFGNode(sgn,  1); else
//    cn = CFGNode(sgn,  2);
//  }
...
//  for(set<FuseCFGNodePtr>::iterator i=ATSNodes.begin(); i!=ATSNodes.end();
//  ++i) {
...
//  for(set<SSAMemLocObjectPtr>::const_iterator ml=defsML.begin();
//  ml!=defsML.end(); ++ml) {
