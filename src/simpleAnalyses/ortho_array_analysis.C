...
EmptyMLType::EmptyMLType(const EmptyMLType &that)
...
bool EmptyMLType::isLiveMLType(PartEdgePtr pedge) { return true; }

// Returns whether this AbstractObject denotes the set of all possible execution
// prefixes.
bool EmptyMLType::isFullMLType(PartEdgePtr pedge) { return false; }
// Returns whether this AbstractObject denotes the empty set.
bool EmptyMLType::isEmptyMLType(PartEdgePtr pedge) { return true; }

// pretty print
std::string EmptyMLType::str(std::string indent) const { return "empty"; }

/******************
 * NonArrayMLType *
 ******************/
NonArrayMLType::NonArrayMLType(MemLocObjectPtr ml)
    : BaseMLType(BaseMLType::notarray), ml(ml) {}
NonArrayMLType::NonArrayMLType(const NonArrayMLType &that)
    : BaseMLType(that.getMType()), ml(that.ml) {}

BaseMLTypePtr NonArrayMLType::copyMLType() const {
  return boost::make_shared<NonArrayMLType>(*this);
}

MemLocObjectPtr NonArrayMLType::getML() const { return ml; }

bool NonArrayMLType::mayEqualMLType(BaseMLTypePtr that, PartEdgePtr pedge) {
  if (that->getMType() == BaseMLType::full)
    return true;
  else if (that->getMType() == BaseMLType::empty)
    return false;
    ...
    NonArrayMLTypePtr thatMT =
        boost::dynamic_pointer_cast<NonArrayMLType>(that);
...
bool NonArrayMLType::mustEqualMLType(BaseMLTypePtr that, PartEdgePtr pedge) {
  if (that->getMType() == BaseMLType::full ||
      ...
      that->getMType() == BaseMLType::array)
    return false;
  else {
    NonArrayMLTypePtr thatMT =
        boost::dynamic_pointer_cast<NonArrayMLType>(that);
...
bool NonArrayMLType::meetUpdateMLType(NonArrayMLTypePtr that,
                                      PartEdgePtr pedge) {
 ...
 * ArrayIndexVector *
 ********************/
ArrayIndexVector::ArrayIndexVector(std::list<ValueObjectPtr> indices)
    : indices(indices) {}
...
bool ArrayIndexVector::mayEqualV(ArrayIndexVectorPtr that, PartEdgePtr pedge) {
  ...
  for (; thisIt != indices.end() && thatIt != indices.end();
    ...
    if (thisV->mayEqualV(thatV, pedge))
      return true;
  }
  return false;
}
...
bool ArrayIndexVector::mustEqualV(ArrayIndexVectorPtr that, PartEdgePtr pedge) {
  ...
  for (; thisIt != indices.end() && thatIt != indices.end();
    ...
    ValueObjectPtr thisV = *thisIt;
    ValueObjectPtr thatV = *thatIt;
    if (!thisV->mustEqualV(thatV, pedge))
      return false;
  }
  return true;
}
...
bool ArrayIndexVector::equalSetV(ArrayIndexVectorPtr that, PartEdgePtr pedge) {
  ...
  for (; thisIt != indices.end() && thatIt != indices.end();
    ...
    ValueObjectPtr thisV = *thisIt;
    ValueObjectPtr thatV = *thatIt;
    if (!thisV->equalSetV(thatV, pedge))
      return false;
  }
  return true;
}
...
bool ArrayIndexVector::subSetV(ArrayIndexVectorPtr that, PartEdgePtr pedge) {
  ...
  for (; thisIt != indices.end() && thatIt != indices.end();
    ...
    ValueObjectPtr thisV = *thisIt;
    ValueObjectPtr thatV = *thatIt;
    if (thisV->subSetV(thatV, pedge))
      return true;
  }
  return false;
}

bool ArrayIndexVector::meetUpdateV(ArrayIndexVectorPtr that,
                                   PartEdgePtr pedge) {
  list<ValueObjectPtr> thatIV = that->getArrayIndexVector();
  bool flag = false;
  list<ValueObjectPtr>::iterator thisIt, thatIt;
...
ArrayMLType::ArrayMLType(SgNode *sgn, MemRegionObjectPtr region,
    ...
    : BaseMLType(BaseMLType::array), array_ref(sgn), region(region),
      indices(indices), origML(origML) {}

ArrayMLType::ArrayMLType(const ArrayMLType &that)
    : BaseMLType(that.getMType()), array_ref(that.array_ref) {
  region = that.region->copyMR();
  indices = that.indices->copyV();
  origML = that.origML->copyML();
}

MemRegionObjectPtr ArrayMLType::getArrayMR() const { return region; }

ArrayIndexVectorPtr ArrayMLType::getArrayIndexVector() const { return indices; }

MemLocObjectPtr ArrayMLType::getOrigML() const { return origML; }
...
bool ArrayMLType::mustEqualMLType(BaseMLTypePtr that, PartEdgePtr pedge) {
  if (that->getMType() == BaseMLType::full ||
      that->getMType() == BaseMLType::empty ||
      that->getMType() == BaseMLType::notarray)
    return false;
  else {
    ArrayMLTypePtr thatMT = boost::dynamic_pointer_cast<ArrayMLType>(that);
    MemRegionObjectPtr thatMR = thatMT->getArrayMR();
    ArrayIndexVectorPtr thatINDICES = thatMT->getArrayIndexVector();
    return region->mustEqualMR(thatMR, pedge) &&
           indices->mustEqualV(thatINDICES, pedge);
  }
}
...
bool ArrayMLType::equalSetMLType(BaseMLTypePtr that, PartEdgePtr pedge) {
  if (that->getMType() == BaseMLType::full ||
    ...
    MemRegionObjectPtr thatMR = thatMT->getArrayMR();
    ArrayIndexVectorPtr thatINDICES = thatMT->getArrayIndexVector();
    return region->equalSetMR(thatMR, pedge) &&
           indices->equalSetV(thatINDICES, pedge);
  }
}
...
bool ArrayMLType::subSetMLType(BaseMLTypePtr that, PartEdgePtr pedge) {
  if (that->getMType() == BaseMLType::full)
    ...
    MemRegionObjectPtr thatMR = thatMT->getArrayMR();
    ArrayIndexVectorPtr thatINDICES = thatMT->getArrayIndexVector();
    return region->subSetMR(thatMR, pedge) &&
           indices->subSetV(thatINDICES, pedge);
  }
}
...
bool FullMLType::isLiveMLType(PartEdgePtr pedge) { return true; }

// Returns whether this AbstractObject denotes the set of all possible execution
// prefixes.
bool FullMLType::isFullMLType(PartEdgePtr pedge) { return true; }
// Returns whether this AbstractObject denotes the empty set.
bool FullMLType::isEmptyMLType(PartEdgePtr pedge) { return false; }

// pretty print
std::string FullMLType::str(std::string indent) const { return "full"; }
...
BaseMLTypePtr ArrayML::getArrayMLType() const { return mltype; }
...
bool ArrayML::mayEqualML(MemLocObjectPtr that, PartEdgePtr pedge) {
  ArrayMLPtr thatML = boost::dynamic_pointer_cast<ArrayML>(that);
  assert(thatML);
...
bool ArrayML::meetUpdateML(MemLocObjectPtr that, PartEdgePtr pedge) {
  ...
  else if (thatMT->BaseMLType::empty)
    return false;
  // If both are non array type
    ...
    NonArrayMLTypePtr thisNAMT =
        boost::dynamic_pointer_cast<NonArrayMLType>(mltype);
    NonArrayMLTypePtr thatNAMT =
        boost::dynamic_pointer_cast<NonArrayMLType>(thatMT);
...
MemLocObjectPtr ArrayAnalysis::Expr2MemLoc(SgNode *n, PartEdgePtr pedge) {
  ...
  if (isSgPntrArrRefExp(n) &&
      (!isSgPntrArrRefExp(n->get_parent()) ||
       !isSgPntrArrRefExp(
           isSgPntrArrRefExp(n->get_parent())->get_lhs_operand()))) {
    ...
    SageInterface::isArrayReference(isSgExpression(n), &arrayNameExp,
                                    &subscripts);
    ...
    MemRegionObjectPtr region =
        getComposer()->Expr2MemRegion(arrayNameExp, pedge, this);
    ...
    for (; it != subscripts->end(); ++it) {
      ...
      SIGHT_VERB(dbg << "index=" << SgNode2Str(index) << endl, 2,
                 arrayAnalysisDebugLevel)
      SIGHT_VERB(dbg << "indexV=" << v->str() << endl, 2,
                 arrayAnalysisDebugLevel)
      subscriptsV.push_back(v->copyV());
    }
    ArrayIndexVectorPtr indices =
        boost::make_shared<ArrayIndexVector>(subscriptsV);
    ArrayMLTypePtr mltype =
        boost::make_shared<ArrayMLType>(n, region, indices, origML);
