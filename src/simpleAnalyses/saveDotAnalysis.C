...
void printPart(std::ostream &o, map<PartPtr, partDotInfoPtr> &partInfo,
               PartPtr part, string indent);
// Edge printer
void printEdge(std::ostream &o, map<PartPtr, partDotInfoPtr> &partInfo,
...
void ats2dot(std::string fName, std::string graphName, set<PartPtr> &startParts,
             set<PartPtr> &endParts) {
...
void ats2dot_bw(std::string fName, std::string graphName,
                set<PartPtr> &startParts, set<PartPtr> &endParts) {
  ofstream out((fName + ".dot").c_str(), std::ofstream::out);
...
// Given a map from PartPtrs to their unique IDs, return the unique ID of the
// given PartPtr, adding a new ID for it if it is not already in the map
...
Ctxt2PartsMap::Ctxt2PartsMap(bool crossAnalysisBoundary,
                             const list<list<PartContextPtr>> &key,
...
// Given a key, pulls off the PartContextPtr at its very front and returns the
// resulting key along with a flag that indicates whether the front element of
// the top-level list was removed in the process
Ctxt2PartsMap::SubKey
Ctxt2PartsMap::getNextSubKey(const list<list<PartContextPtr>> &key) {
  assert(key.size() > 0);
  ...
  // If the new key's first element is now empty, remove it as well and record
  // this fact
...
// Returns the total number of PartContextPtr within key, across all the
// second-level lists
int Ctxt2PartsMap::getNumSubKeys(const list<list<PartContextPtr>> &key) {
  int total = 0;
  for (list<list<PartContextPtr>>::const_iterator i = key.begin();
       i != key.end(); i++) {
    assert(i->size() > 0);
    total += i->size();
  }
...
void Ctxt2PartsMap::insert(const list<list<PartContextPtr>> &key,
                           PartPtr part) {
  ...
  if (getNumSubKeys(key) == 1)
    ...
    if (m.find(sk.front) == m.end())
      m[sk.front] = mgen->newMap(false, mgen, lgen);
...
void Ctxt2PartsMap::map2dot(std::ostream &o,
  ...
  for (std::map<PartContextPtr, Ctxt2PartsMap *>::const_iterator c = m.begin();
       c != m.end(); c++, i++) {
    ostringstream subsubgraphName;
    subsubgraphName << subgraphName << "_N" << i;
    o << indent << "subgraph " << subsubgraphName.str() << "{" << endl;
    o << indent << "  color=" << (crossAnalysisBoundary ? "red" : "black")
      << ";" << endl;
  ...
  i=0;
  for(std::map<PartContextPtr, Ctxt2PartsMap*>::const_iterator c=m.begin();
  c!=m.end(); c++, i++) { ostringstream subsubgraphName; subsubgraphName <<
  subgraphName<<"_"<<i; o << subsubgraphName.str()<<"; ";
  }
  o << "};"<<endl;*/
}
...
std::string Ctxt2PartsMap::str(std::string indent) const {
  ...
  for (map<PartContextPtr, Ctxt2PartsMap *>::const_iterator i = m.begin();
       i != m.end(); i++) {
  ...
  /*if(m.size()>1) {
    ...
    for(map<PartContextPtr, Ctxt2PartsMap* >::iterator j=m.begin(); j!=i; j++) {
      oss << indent << "==" << (i->first==j->first) << "
  i="<<i->first.get()->str(indent+"  ")<<" j="<<j->first.get()->str(indent+"
  ")<<endl; } }
  }*/
  oss << "]";
  return oss.str();
...
Ctxt2PartsMap_Leaf::Ctxt2PartsMap_Leaf(const list<list<PartContextPtr>> &key,
                                       PartPtr part) {
  insert(key, part);
}

// Stores the given PartPtr under the given Context key.
void Ctxt2PartsMap_Leaf::insert(const list<list<PartContextPtr>> &key,
                                PartPtr part) {
  assert(Ctxt2PartsMap::getNumSubKeys(key) == 1);
  Ctxt2PartsMap::SubKey sk = Ctxt2PartsMap::getNextSubKey(key);
  // cout << "Ctxt2PartsMap_Leaf::insert() <<#m="<<m.size()<<"
  // key="<<(sk.front.get()->str())<<endl;
...
set<PartPtr> Ctxt2PartsMap_Leaf::get(const list<list<PartContextPtr>> &key) {
  assert(Ctxt2PartsMap::getNumSubKeys(key) == 1);
  // if(m.find(*(key.begin())) == m.end() || m[*(key.begin())].find(loc) ==
  // m[*(key.begin())].end()) {
  Ctxt2PartsMap::SubKey sk = Ctxt2PartsMap::getNextSubKey(key);
  if (m.find(sk.front) == m.end())
    return set<PartPtr>();
...
// display them in the dot graph subgraphName: name of the subgraph that
// contains the current level in the map
void Ctxt2PartsMap_Leaf::map2dot(std::ostream &o,
                                 map<PartPtr, partDotInfoPtr> &partInfo,
                                 std::string subgraphName,
                                 std::string indent) const {
  // cout << indent << "Ctxt2PartsMap_Leaf::map2dot()
  // subgraphName="<<subgraphName<<" #m="<<m.size()<<endl;
  ...
  for (map<PartContextPtr, set<PartPtr>>::const_iterator c = m.begin();
       c != m.end(); c++, i++) {
    ostringstream subsubgraphName;
    subsubgraphName << subgraphName << i;
    o << indent << "subgraph " << subsubgraphName.str() << "{" << endl;
    ...
    for (set<PartPtr>::iterator p = c->second.begin(); p != c->second.end();
      ...
      if (p->get()->mayOutgoingFuncCall(matchNodes))
    ...
    for (set<PartPtr>::iterator in = funcCallsIn.begin();
      ...
      o << indent << "subgraph " << subsubgraphName.str() << "_Call" << inIdx
        << "{" << endl;
      ...
      for (set<PartPtr>::iterator out = funcCallsOut.begin();
        ...
        if (in->get()->mayMatchFuncCall(*out)) {
          o << indent << getPartUID(partInfo, *out) << " -> "
            << getPartUID(partInfo, *in) << "[style=invis]" << endl;
          o << indent << getPartUID(partInfo, *out) << endl;
          sink << getPartUID(partInfo, *out)
               << (numMatchedCalls == 0 ? "" : ",");
          numMatchedCalls++;
        }
      }
      o << indent << "  { rank=sink; " << getPartUID(partInfo, *in) << " }"
        << endl;
...
std::string Ctxt2PartsMap_Leaf::str(std::string indent) const {
  ...
  for (map<PartContextPtr, set<PartPtr>>::const_iterator i = m.begin();
       i != m.end(); i++) {
    oss << indent << i->first.get()->str(indent + "  ") << " =&gt; " << endl;
    for (set<PartPtr>::iterator j = i->second.begin(); j != i->second.end();
         j++)
...
std::ostream &ats2dot(std::ostream &o, std::string graphName,
                      set<PartPtr> &startParts, set<PartPtr> &endParts) {
#ifndef DISABLE_SIGHT
  ...
  Ctxt2PartsMap ctxt2parts(
      false, boost::make_shared<Ctxt2PartsMap_Generator_Base>(),
  ...
  for(fw_partEdgeIterator state(startParts); state!=fw_partEdgeIterator::end();
  state++) {*/
#ifndef DISABLE_SIGHT
  {
    scope s("startParts");
    for (set<PartPtr>::const_iterator p = startParts.begin();
         p != startParts.end(); ++p)
      dbg << (*p)->str() << endl;
  }
#endif
  ...
  for (fw_partEdgeIterator state(startParts, /*incrementalGraph*/ false);
...
#ifndef DISABLE_SIGHT
    scope reg2(sight::txt() << "ats2dot: part=" << getPartUID(partInfo, part)
                            << "=" << part->str(),
               scope::medium, attrGE("saveDotAnalysisDebugLevel", 1));
    if (saveDotAnalysisDebugLevel() >= 1) {
      dbg << "context=" << part->getContext()->str() << endl;
      dbg << "pedge=" << state.getPartEdge()->str() << endl;

      /*{ scope s("partInfo");
      for(map<PartPtr, partDotInfoPtr>::iterator i=partInfo.begin();
      i!=partInfo.end(); ++i) { dbg << i->first->str()<<endl; } }*/
    }
#endif
    ...
    { / *dbg << "edge ="<<getPartUID(partInfo,
    (*e)->source())<<"="<<(*e)->source()->str()<< " =&gt; " <<
                            getPartUID(partInfo,
    (*e)->target())<<"="<<(*e)->target()->str()<<endl;* / printEdge(edgesStr,
    partInfo, *e, false, "  ");* /

      // If the edges's source and target have different contexts, the target
    must be an entry node of its context
      ...
      // If the target of this edge has not yet been visited, add it to the
    iterator / *if(visited.find((*e)->target()) != visited.end())
        state.add((*e)->target());* /
    }*/
  }
  ...
  /*int i=0;
  for(map<ContextPtr, set<PartPtr> >::iterator c=ctxt2parts.begin();
  c!=ctxt2parts.end(); c++, i++) { o << "subgraph cluster_"<<i<<"{"<<endl; o <<
  "    color=lightgrey;"<<endl; o << "    style=filled;"<<endl; o << "    label
  = \""<<c->first.get()->str()<<"\";"<<endl; for(set<PartPtr>::iterator
  p=c->second.begin(); p!=c->second.end(); p++) { printPart(o, partInfo, *p);
    }

    // Make sure that all the entry parts are on top of their context
    / *o << "    { rank=min; ";
    map<ContextPtr, set<PartPtr> >::iterator entr =
  ctxt2EntryParts.find(c->first); for(set<PartPtr>::iterator
  p=entr->second.begin(); p!=entr->second.end(); p++) o << getPartUID(partInfo,
  *p)<<"; "; o << "};"<<endl;
...
std::ostream &ats2dot_bw(std::ostream &o, std::string graphName,
                         set<PartPtr> &startParts, set<PartPtr> &endParts) {
#ifndef DISABLE_SIGHT
  ...
  // for(bw_partEdgeIterator state(endParts); state!=bw_partEdgeIterator::end();
  // state++) {
  for (bw_partEdgeIterator state(endParts, /*incrementalGraph*/ false);
       !state.isEnd(); state++) {
    PartPtr part = state.getPart();
// cout << "==== part="<<part->str()<<endl;
#ifndef DISABLE_SIGHT
    scope reg(sight::txt() << "ats2dot: part=" << getPartUID(partInfo, part)
                           << "=" << part->str(),
              scope::medium, attrGE("saveDotAnalysisDebugLevel", 1));
    if (saveDotAnalysisDebugLevel() >= 1) {
      dbg << "state=" << state.str() << endl;
      dbg << "*state=" << part->str()
          << " context=" << part->getContext()->str() << endl;
      dbg << "pedge=" << state.getPartEdge()->str() << endl;
    }
#endif
...
void printPart(std::ostream &o, map<PartPtr, partDotInfoPtr> &partInfo,
  ...
  // Function entry/exit
  set<CFGNode> matchNodes;
  if (part->mayFuncEntry(matchNodes) || part->mayFuncExit(matchNodes))
    nodeStyle = "dotted";
  ...
  o << escapeDOT(labelMultLineStr) << "\", color=\"" << nodeColor
    << "\", fillcolor=\"white\", style=\"" << nodeStyle << "\", shape=\""
    << nodeShape << "\"];\n";
}
...
void printEdge(std::ostream &o, map<PartPtr, partDotInfoPtr> &partInfo,
  ...
  if (pv.size() > 0) {
    ...
    for (map<CFGNode, boost::shared_ptr<SgValueExp>>::iterator v = pv.begin();
         v != pv.end(); v++) {
      if (v != pv.begin())
        values << ", ";
  ...
  o << indent << getPartUID(partInfo, e->source()) << " -> "
    << getPartUID(partInfo, e->target()) << " [label=\""
    << escapeDOT(values.str()) << "\","
