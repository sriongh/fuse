...
PointsToAnalysisTransfer::PointsToAnalysisTransfer(
    ...
    std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo, Composer *_composer,
    PointsToAnalysis *_analysis)
    : DFTransferVisitor(parts, cn, state, dfInfo), composer(_composer),
      analysis(_analysis), modified(false) {
  // Set the pointer of the map for this PartEdge
  initLattice();
}

bool PointsToAnalysisTransfer::finish() { return modified; }
...
PointsToAnalysisTransfer::getLattice(SgExpression *sgexp) {
  MemLocObjectPtr ml = composer->Expr2MemLoc(
      sgexp, parts.NodeState()->inEdgeFromAny(), analysis);
  return getLatticeCommon(ml);
}

PointsToAnalysisTransfer::AbstractObjectSetPtr
PointsToAnalysisTransfer::getLatticeOperand(SgNode *sgn,
                                            SgExpression *operand) {
  MemLocObjectPtr oml = composer->OperandExpr2MemLoc(
      sgn, operand, parts.NodeState()->inEdgeFromAny(), analysis);
  return getLatticeCommon(oml);
}

PointsToAnalysisTransfer::AbstractObjectSetPtr
PointsToAnalysisTransfer::getLatticeCommon(MemLocObjectPtr ml) {
  return boost::dynamic_pointer_cast<AbstractObjectSet>(
      productLattice->get(ml));
}

bool PointsToAnalysisTransfer::setLattice(
    SgExpression *sgexp, PointsToAnalysisTransfer::AbstractObjectSetPtr lat) {
  MemLocObjectPtr ml = composer->Expr2MemLoc(
      sgexp, parts.NodeState()->inEdgeFromAny(), analysis);
  return setLatticeCommon(ml, lat);
}

bool PointsToAnalysisTransfer::setLatticeOperand(
    SgNode *sgn, SgExpression *operand,
    PointsToAnalysisTransfer::AbstractObjectSetPtr lat) {
  MemLocObjectPtr ml = composer->OperandExpr2MemLoc(
      sgn, operand, parts.NodeState()->inEdgeFromAny(), analysis);
  return setLatticeCommon(ml, lat);
}

bool PointsToAnalysisTransfer::setLatticeCommon(
    MemLocObjectPtr ml, PointsToAnalysisTransfer::AbstractObjectSetPtr lat) {
...
void PointsToAnalysisTransfer::visit(SgAssignOp *sgn) {
  ...
  // Handle p = &x
  // NOTE: rhs can be a complex expression but code below only handles the
  // trivial case
  if (isSgAddressOfOp(rhs_operand)) {
    // Operand of SgAddressOfOp should be a variable
    SgVarRefExp *sgvexp =
        isSgVarRefExp(isSgAddressOfOp(rhs_operand)->get_operand());
    assert(sgvexp);
    MemLocObjectPtr ml = composer->OperandExpr2MemLoc(
        sgn, sgvexp, parts.NodeState()->inEdgeFromAny(), analysis);
    assert(ml);
    ...
    // If a new element is inserted into set, AbstractObjectMap corresponding to
    // lhs needs update. AbstractObjetMap::get only returns a copy of the set.
...
void PointsToAnalysisTransfer::visit(SgPointerDerefExp *sgn) {
  // scope reg(sight::txt()<<"PointsToAnalysisTransfer::visit(sgn=" <<
  // SgNode2Str(sgn) << ")", scope::medium, attrGE("pointsToAnalysisDebugLevel",
  // 2)); AbstractObjectSetPtr operandLat = getLatticeOperand(sgn,
  // sgn->get_operand()); MemLocObjectPtr opML_p =
  // composer->OperandExpr2MemLoc(sgn, sgn->get_operand(),
  // part->inEdgeFromAny(), analysis);
  // // MemLocObjectPtr ml_p = composer->Expr2MemLoc(sgn, part->inEdgeFromAny(),
  // analysis); if(pointsToAnalysisDebugLevel() >= 2) {
...
void PointsToAnalysis::genInitLattice(const AnalysisParts &parts,
                                      const AnalysisPartEdges &pedges,
                                      std::vector<Lattice *> &initLattices) {
  AbstractObjectMap *productlattice = new AbstractObjectMap(
      boost::make_shared<AbstractObjectSet>(pedges.NodeState(), getComposer(),
                                            this, AbstractObjectSet::may),
      pedges.NodeState(), getComposer(), this);
  initLattices.push_back(productlattice);
}

boost::shared_ptr<DFTransferVisitor> PointsToAnalysis::getTransferVisitor(
    AnalysisParts &parts, CFGNode cn, NodeState &state,
    std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo) {
  PointsToAnalysisTransfer *ptat = new PointsToAnalysisTransfer(
...
#ifndef DISABLE_SIGHT
  scope reg(sight::txt() << "PointsToAnalysis::Expr2MemLoc(sgn="
                         << SgNode2Str(sgn) << ")",
            scope::medium, attrGE("pointsToAnalysisDebugLevel", 2));
  if (pointsToAnalysisDebugLevel() >= 2) {
    dbg << "pedge=" << pedge->str() << endl;
  }
#endif
  // ML object returned by Pointsto analysis
  boost::shared_ptr<PTMemLocObject> ptML_p =
      boost::make_shared<PTMemLocObject>(pedge, getComposer(), this);

  // Switch based on the type of the expression
  switch (sgn->variantT()) {
  // Handle all pointer dereference expressions by looking up analysis state
  case V_SgPointerDerefExp: {
    MemLocObjectPtr opML_p = getComposer()->OperandExpr2MemLoc(
        sgn, isSgPointerDerefExp(sgn)->get_operand(), pedge, this);
    ...
    boost::shared_ptr<AbstractObjectSet> aos_p =
        boost::dynamic_pointer_cast<AbstractObjectSet>(aom_p->get(opML_p));
    assert(!aos_p->isEmpty());
#ifndef DISABLE_SIGHT
    if (pointsToAnalysisDebugLevel() >= 2)
      dbg << "MLSet=" << aos_p->str() << endl;
#endif
...
PTMemLocObject::PTMemLocObject(PartEdgePtr pedge, Composer *composer,
                               PointsToAnalysis *ptanalysis)
    : MemLocObject(NULL), ptanalysis(ptanalysis) {
  aos_p = boost::make_shared<AbstractObjectSet>(pedge, composer, ptanalysis,
...
MemRegionObjectPtr PTMemLocObject::getRegion() const {
  // Collect the MemRegions of all the MemLocs in aos and create from a
  // UnionMemRegion
  if (region == NULLMemRegionObject) {
    std::list<MemRegionObjectPtr> memRegions;
    for (AbstractObjectSet::const_iterator ml = aos_p->begin();
         ml != aos_p->end(); ml++) {
      MemLocObjectPtr curML = boost::dynamic_pointer_cast<MemLocObject>(*ml);
      memRegions.push_back(curML->getRegion());
    }
    ((PTMemLocObject *)this)->region =
        boost::make_shared<CombinedMemRegionObject>(Union, ptanalysis,
                                                    memRegions);
  }
  return region;
}
...
ValueObjectPtr PTMemLocObject::getIndex() const {
  if (index == NULLValueObject) {
    // Collect all the indexes of the memLocs in this object and create a
    ...
    ((PTMemLocObject *)this)->index =
        boost::make_shared<CombinedValueObject>(Union, ptanalysis, indexes);
  }
  return index;
}
...
void PTMemLocObject::add(boost::shared_ptr<AbstractObjectSet> thataos_p,
...
bool PTMemLocObject::mayEqual(MemLocObjectPtr thatML_p, PartEdgePtr pedge,
                              Composer *comp, ComposedAnalysis *analysis) {
  // scope reg(sight::txt()<<"PTMemLocObject::mayEqualML", scope::medium,
  // attrGE("pointsToAnalysisDebugLevel", 2));
  PTMemLocObjectPtr thatPTML_p =
      boost::dynamic_pointer_cast<PTMemLocObject>(thatML_p);
  assert(thatPTML_p);

  if (isFull(pedge, comp, analysis) ||
      thatPTML_p->isFull(pedge, comp, analysis))
    return true;
  ...
  for (; cIt != thatMLSet.end(); ++cIt) {
    if (aos_p->containsMay(boost::static_pointer_cast<AbstractObject>(*cIt)))
      return true;
  }
  return false;
}
...
bool PTMemLocObject::mustEqual(MemLocObjectPtr thatML_p, PartEdgePtr pedge,
                               Composer *comp, ComposedAnalysis *analysis) {
  PTMemLocObjectPtr thatPTML_p =
      boost::dynamic_pointer_cast<PTMemLocObject>(thatML_p);
  assert(thatPTML_p);
  ...
  if (aos_p->size() != 1 || thatMLSet.size() != 1)
    return false;

  assert(aos_p->size() == (thatMLSet.size() == 1));

  // The object in the set should also mustEqual each other
  const AbstractObjectPtr thatAO_p =
      boost::static_pointer_cast<AbstractObject>(*thatMLSet.begin());

  return aos_p->containsMust(thatAO_p);
}

bool PTMemLocObject::equalSet(MemLocObjectPtr thatML_p, PartEdgePtr pedge,
                              Composer *comp, ComposedAnalysis *analysis) {
  PTMemLocObjectPtr thatPTML_p =
      boost::dynamic_pointer_cast<PTMemLocObject>(thatML_p);
  assert(thatPTML_p);
...
bool PTMemLocObject::subSet(MemLocObjectPtr thatML_p, PartEdgePtr pedge,
                            Composer *comp, ComposedAnalysis *analysis) {
  PTMemLocObjectPtr thatPTML_p =
      boost::dynamic_pointer_cast<PTMemLocObject>(thatML_p);
  assert(thatPTML_p);
  ...
  for (cIt = cbegin; cIt != cend; ++cIt) {
    // If a single element in the smaller set not equals any element in the
    // larger set then the subSet relation does not hold.
    if (!laos_p->containsEqualSet(*cIt))
      return false;
  }
  return true;
}

bool PTMemLocObject::isLive(PartEdgePtr pedge, Composer *comp,
                            ComposedAnalysis *analysis) {
  if (aos_p->size() == 0)
    return false;
  ...
  for (; cIt != aos_p->end(); ++cIt) {
    ...
    if (cML_p->isLive(pedge, comp, analysis))
      return true;
  }
  return false;
}

bool PTMemLocObject::meetUpdate(MemLocObjectPtr thatML_p, PartEdgePtr pedge,
                                Composer *comp, ComposedAnalysis *analysis) {
  PTMemLocObjectPtr thatPTML_p =
      boost::dynamic_pointer_cast<PTMemLocObject>(thatML_p);
  assert(thatPTML_p);
...
bool PTMemLocObject::isFull(PartEdgePtr pedge, Composer *comp,
                            ComposedAnalysis *analysis) {
  return aos_p->isFull();
}

bool PTMemLocObject::isEmpty(PartEdgePtr pedge, Composer *comp,
                             ComposedAnalysis *analysis) {
...
// {
//   scope regvis("Expr2MemLocTraversal::visit(SgPointerDerefExp* sgn)",
//   scope::medium, attrGE("pointsToAnalysisDebugLevel", 1)); SgExpression*
//   operand = sgn->get_operand(); operand->accept(*this);
//   boost::shared_ptr<AbstractObjectSet> new_p_aos =
//           boost::make_shared<AbstractObjectSet>(pedge, composer, analysis,
//           AbstractObjectSet::may);
//   for(AbstractObjectSet::const_iterator i=p_aos->begin(); i!=p_aos->end();
//   i++) {
//     boost::shared_ptr<AbstractObjectSet> ao =
//     boost::dynamic_pointer_cast<AbstractObjectSet>(aom->get(*i)); assert(ao);
...
// {
//   scope regvis("Expr2MemLocTraversal::visit(SgVarRefExp* sgn)",
//   scope::medium, attrGE("pointsToAnalysisDebugLevel", 1)); dbg <<
//   "isSgPointerType(sgn->get_type())="<<isSgPointerType(sgn->get_type())<<endl;
...
//   }*/
//   p_aos = boost::make_shared<AbstractObjectSet>(pedge, composer, analysis,
//   AbstractObjectSet::may); p_aos->insert(composer->Expr2MemLoc(sgn, pedge,
//   analysis)); if(pointsToAnalysisDebugLevel()>=1) dbg <<
//   "p_aos="<<p_aos->str()<<endl;
// }
