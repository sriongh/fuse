...
namespace fuse {
ConstPropCountAnalysis::ConstPropCountAnalysis()
    : FWDataflow(false, false), m_assignExpCount(0), m_unassignExpCount(0),
      m_knownCondCount(0), m_unknownCondCount(0), m_edges(0) {
...
ComposedAnalysisPtr ConstPropCountAnalysis::copy() {
  ...
  return boost::make_shared<ConstPropCountAnalysis>();
}

ValueObjectPtr ConstPropCountAnalysis::Expr2Val(SgNode *sgn,
                                                PartEdgePtr pedge) {
  assert(false);
}

bool ConstPropCountAnalysis::implementsExpr2Val() { return false; }

void ConstPropCountAnalysis::
...
bool ConstPropCountAnalysis::countConst(
  ...
  //     ValueObjectPtr v = getComposer()->OperandExpr2Val(assignOp, assignOp,
  //     parts.NodeState()->outEdgeToAny(), this);
  //     // cout << v->str() << endl;
  //     if(v->isConcrete()) {
  //       m_assignExpCount++;
  //     }
  ...
  //     isSgExprStatement(ifstmt->get_conditional()); if(exprStmt) {
  //       SgExpression* expr = exprStmt->get_expression();
  //       ValueObjectPtr v = getComposer()->OperandExpr2Val(expr, expr,
  //       parts.NodeState()->outEdgeToAny(), this); if(v->isConcrete()) {
  //         m_knownCondCount++;
  //       }
  //       else {
  //         m_unknownCondCount++;
  ...
  //     }
  //     else {
  //       cerr << "Warning: SgIfStmt()->get_conditional() !=
  //       SgExprStatement\n";
  //     }
  //   }
  // }
...
bool ConstPropCountAnalysis::transfer(
    AnalysisParts &parts, CFGNode cn, NodeState &state,
    std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo) {
  m_edges++;

  PartPtr part = parts.index();
  list<PartEdgePtr> outEdges = parts.index()->outEdges();

  nodess << indent << part2dot(part);

  list<PartEdgePtr>::iterator oe = outEdges.begin();
  for (; oe != outEdges.end(); oe++) {
...
std::string ConstPropCountAnalysis::cfgn2label(CFGNode cfgn) {
  ostringstream oss;
  string node_s;
...
string ConstPropCountAnalysis::part2label(PartPtr part) {
...
string ConstPropCountAnalysis::newcontext2str() {
  ostringstream oss;
  oss << "cc" << contextid++;
  return oss.str();
}

string ConstPropCountAnalysis::context2dotid(PartContextPtr pcontext) {
...
string ConstPropCountAnalysis::part2dotid(PartPtr part) {
  set<CFGNode> cfgnset = part->CFGNodes();
...
string ConstPropCountAnalysis::part2dot(PartPtr part) {
  ostringstream oss;
  string nodeColor = "black";
  if (part->mustSgNodeAll<SgStatement>())
    nodeColor = "blue";
  else if (part->mustSgNodeAll<SgExpression>())
    nodeColor = "darkgreen";
  else if (part->mustSgNodeAll<SgInitializedName>())
    nodeColor = "brown";
  // First write the id
  oss << part2dotid(part) << " ";
...
string ConstPropCountAnalysis::partedge2dot(PartEdgePtr pedge) {
  ostringstream oss;
...
void ConstPropCountAnalysis::generateDot() {
  // Composer* composer = analysis->getComposer();
  // set<PartPtr> initial = composer->GetStartAStates(analysis);
  // fw_dataflowGraphEdgeIterator<PartEdgePtr, PartPtr> ei(/* not working on
  // incremental graph */
  //     						  false,
...
void ConstPropCountAnalysis::generateDotFile(int instance) {
  ofstream file;
  SgProject *project = SageInterface::getProject();
  SgFile *sgfile = project->get_files()[0];
  assert(sgfile);
  ostringstream namess;
  namess << Rose::StringUtility::stripPathFromFileName(sgfile->getFileName())
         << "-ats-" << instance << ".dot";
