 ...
 ***************************/
FuseMultiCPAstAttribute::FuseMultiCPAstAttribute(SgNode *sgn)
    : AstAttribute(), sgn(sgn) {}

FuseMultiCPAstAttribute::FuseMultiCPAstAttribute(SgNode *sgn,
                                                 MemLocObjectPtr ml_)
    : AstAttribute(), sgn(sgn), ml(ml_->copyAOType()) {}

FuseMultiCPAstAttribute::FuseMultiCPAstAttribute(
    const FuseMultiCPAstAttribute &that)
...
FuseMultiCPAnnotMap FuseMultiCPAstAttribute::getAnnotMap() { return valuesMap; }
...
void addFuseAnnot(SgAssignOp *sgn, CPValueLatticePtr vlat, Analysis *analysis) {
  ...
  if (sgn->attributeExists("fuse:FuseMultiCPAstAttribute")) {
    attr = dynamic_cast<FuseMultiCPAstAttribute *>(
        sgn->getAttribute("fuse:FuseMultiCPAstAttribute"));
  } else {
    attr = new FuseMultiCPAstAttribute(sgn);
    sgn->addNewAttribute("fuse:FuseMultiCPAstAttribute", attr);
  }

  ValueObjectPtr val = boost::make_shared<CPValueObject>(vlat);
  assert(val);
  attr->add((Analysis *)analysis, val);
  sgn->updateAttribute("fuse:FuseMultiCPAstAttribute", attr);
}

void addFuseAnnot(SgCompoundAssignOp *sgn, CPValueLatticePtr vlat,
                  Analysis *analysis) {
  assert(vlat);
  FuseMultiCPAstAttribute *attr;
  if (sgn->attributeExists("fuse:FuseMultiCPAstAttribute")) {
 ...
 * FuseAnnotTraversal *
 **********************/
FuseAnnotTraversal::FuseAnnotTraversal(list<ComposedAnalysis *> sanalyses_)
    : nassign(0), sanalyses(sanalyses_) {}
...
list<ComposedAnalysis *> FuseAnnotTraversal::getConstantPropagation() {
  ...
  for (; c != sanalyses.end(); ++c) {
    ConstantPropagationAnalysis *cp =
        dynamic_cast<ConstantPropagationAnalysis *>(*c);
    if (cp)
      cpAnalyses.push_back(cp);
  }
  return cpAnalyses;
}
...
void FuseAnnotTraversal::visit(SgNode *sgn) {
  if (isSgAssignOp(sgn) || isSgCompoundAssignOp(sgn)) {
    ...
    if (sgn->attributeExists("fuse:FuseMultiCPAstAttribute")) {
      SIGHT_VERB_DECL(
          scope, (sight::txt() << "sgn=" << SgNode2Str(sgn), scope::medium), 2,
          AnalysisAstAttributeDL)
      FuseMultiCPAstAttribute *attr = dynamic_cast<FuseMultiCPAstAttribute *>(
          sgn->getAttribute("fuse:FuseMultiCPAstAttribute"));
      ...
      for (; c != cpAnalyses.end(); ++c) {
        ...
        if (f != attr->vMapEnd()) {
          ...
          if (val->isConcrete()) {
            ...
            if (assignOpConstantFirstAnalysisCountMap.find(*c) ==
                assignOpConstantFirstAnalysisCountMap.end())
...
void FuseAnnotTraversal::printAssignOpConstantFirstAnalysisCount() {
  std::map<ComposedAnalysis *, int>::iterator c =
      assignOpConstantFirstAnalysisCountMap.begin();
