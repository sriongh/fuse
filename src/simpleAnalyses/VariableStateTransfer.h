...
public:
  ...
  // Returns a Lattice object that corresponds to the memory location denoted by
  // sgn in the current part
  LatticePtr getLattice(SgExpression *sgn) {
    assert(sgn);
    // MemLocObjectPtrPair p = composer->Expr2MemLoc(sgn, part->inEdgeFromAny(),
    // analysis);
    // MemLocObjectPtr p = composer->Expr2MemLoc(sgn, part->inEdgeFromAny(),
    // analysis);
    MemLocObjectPtr p =
        analysis->Expr2MemLocUse(sgn, parts.NodeState()->inEdgeFromAny());
    SIGHT_VERB(dbg << "VariableStateTransfer::getLattice() p="
                   << p->str("&nbsp;&nbsp;&nbsp;&nbsp;") << endl,
               1, dLevel)

    return getLattice(AbstractObjectPtr(p));
    /*ValueObjectPtr val = composer->Expr2Val(sgn, part->inEdgeFromAny(),
    analysis); if(dLevel()>=1) dbg << "VariableStateTransfer::getLattice()
    val="<<val->str("&nbsp;&nbsp;&nbsp;&nbsp;")<<endl; UnionValueObjectPtr
    unionVal = boost::dynamic_pointer_cast<UnionValueObject>(val);
    assert(unionVal->getVals().size()==1);
    LatticePtr cpVal =
    boost::dynamic_pointer_cast<LatticeType>(*unionVal->getVals().begin());
    assert(cpVal);
    return cpVal;*/
  }

  // Returns a Lattice object that corresponds to the memory location denoted by
  // the given operand of sgn in the current part
  LatticePtr getLatticeOperand(SgNode *sgn, SgExpression *operand) {
    SIGHT_VERB_DECL(
        scope, ("VariableStateTransfer::getLatticeOperand()", scope::medium), 1,
        dLevel)
    ...
    // MemLocObjectPtr p = composer->OperandExpr2MemLoc(sgn, operand,
    // part->inEdgeFromAny(), analysis);
    MemLocObjectPtr p = analysis->OperandExpr2MemLocUse(
        sgn, operand, parts.NodeState()->inEdgeFromAny());
    // gettimeofday(&oEnd, NULL); cout << "getLatticeOperand
    // OperandExpr2MemLocUse\t"<<(((oEnd.tv_sec*1000000 + oEnd.tv_usec) -
    // (oStart.tv_sec*1000000 + oStart.tv_usec)) /
    ...
    if (p) {
      ...
      // gettimeofday(&oEnd, NULL); cout << "getLatticeOperand
      // getLattice\t"<<(((oEnd.tv_sec*1000000 + oEnd.tv_usec) -
      // (oStart.tv_sec*1000000 + oStart.tv_usec)) /
      // 1000000.0)<<"\t"<<SgNode2Str(sgn)<<endl;
    ...
    /*ValueObjectPtr val = composer->OperandExpr2Val(sgn, operand,
    part->inEdgeFromAny(), analysis); if(dLevel()>=1) dbg <<
    "VariableStateTransfer::getLatticeOperand()
    val="<<val->str("&nbsp;&nbsp;&nbsp;&nbsp;")<<endl; UnionValueObjectPtr
    unionVal = boost::dynamic_pointer_cast<UnionValueObject>(val);
    assert(unionVal->getVals().size()==1);
    LatticePtr cpVal =
  ...
  // Common code for getLattice() and getLatticeOperand() that returns either
  // the lattice of the expression or memory MemLocObject depending on the type
  // of sgn.
  /*LatticePtr getLatticeCommon(SgExpression* sgn, MemLocObjectPtr p) {
    // For array index expressions, get the lattice associated with the memory
  location
    // since the only content of this expression is what's stored in memory,
  just like with SgVarRefExp
    // if(isSgPntrArrRefExp(sgn))
    //   return getLattice(AbstractObjectPtr(p.expr));
    // else
    //   // Return the lattice associated with n's expression since that is
  likely to be more precise
    //   // but if it is not available, used the memory object
    //   return (p.expr ? getLattice(AbstractObjectPtr(p.expr)) :
  getLattice(AbstractObjectPtr(p.mem))); return
  getLattice(AbstractObjectPtr(p));
  }*/

  LatticePtr getLattice(const AbstractObjectPtr o) {
    LatticePtr l = boost::dynamic_pointer_cast<LatticeType>(prodLat->get(o));
    SIGHT_VERB(dbg << "getLattice(o="
                   << o->strp(parts.NodeState()->inEdgeFromAny(), "")
                   << ", l=" << l->str("") << endl,
               1, dLevel)
    assert(l);
    return l;
  ...
  // Adds prodLat a mapping of the memory location denoted by sgn in the current
  // part to lat. Returns true if this causes prodLat to change and false
  // otherwise.
  void setLattice(SgNode *sgn, LatticePtr lat) {
    ...
    // analysis);
    MemLocObjectPtr p =
        analysis->Expr2MemLocDef(sgn, parts.NodeState()->inEdgeFromAny());
    SIGHT_VERB_IF(1, dLevel)
#ifndef DISABLE_SIGHT
    scope s("setLattice()");
    dbg << "edge=" << parts.NodeState()->inEdgeFromAny()->str() << endl;
    dbg << "p="
        << p->strp(parts.NodeState()->inEdgeFromAny(),
                   "&nbsp;&nbsp;&nbsp;&nbsp;")
        << endl;
  ...
  // Adds prodLat a mapping of the memory location denoted by the given operand
  // of node sgn in the current part to lat. Returns true if this causes prodLat
  ...
  void setLatticeOperand(SgNode *sgn, SgExpression *operand, LatticePtr lat) {
    ...
    // MemLocObjectPtr p = composer->OperandExpr2MemLoc(sgn, operand,
    // part->inEdgeFromAny(), analysis);
    MemLocObjectPtr p = analysis->OperandExpr2MemLocDef(
        sgn, operand, parts.NodeState()->inEdgeFromAny());
    SIGHT_VERB_IF(1, dLevel)
#ifndef DISABLE_SIGHT
  ...
  /*void setLatticeCommon(SgNode* sgn, MemLocObjectPtr p, LatticePtr lat) {
    ...
    // }
    // if(p.mem) {
    //   // If we've already used lat to set p.expr, we need to make a copy of
  it for p.mem
    //   if(p.expr) {
    //     LatticePtr latCopy(dynamic_cast<LatticeType*>(lat->copy()));
    //     lat = latCopy;
  ...
  void setLattice(const AbstractObjectPtr o, LatticePtr lat) {
    // if(dLevel()>=1) dbg << "setLattice(o="<<o->strp(part->inEdgeFromAny(),
    // "")<<", lat="<<lat->strp(part->inEdgeFromAny(), "")<<endl;
    updateModified(prodLat->insert(o, lat));
    // if(dLevel()>=1) dbg <<
    // "&nbsp;&nbsp;&nbsp;prodLat="<<prodLat->str("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")<<endl;
  }

  bool getLattices(SgBinaryOp *sgn, LatticePtr &arg1Lat,
                   LatticePtr &arg2Lat /*, LatticePtr &resLat*/) {
    if (!isSgAssignOp(sgn))
  ...
  bool getLattices(
      SgUnaryOp *sgn,
      LatticePtr &arg1Lat /*, LatticePtr &arg2Lat, LatticePtr &resLat*/) {
    ...
    // if(dLevel()>=1)  {
    // dbg << "res="<<res.str()<<" arg1="<<arg1.str()<<" arg1Lat="<<arg1Lat<<",
    // arg2Lat="<<arg2Lat<<"\n"; dbg << "transfer B, resLat="<<resLat<<"\n";
    //  }

    return (arg1Lat.get() /* && arg2Lat && resLat*/);
  }
...
public:
  VariableStateTransfer(
      ...
      // A pointer to a default example lattice that can be duplicated
      // via defaultLat->copy() to make more instances of this Lattice type.
      LatticePtr defaultLat, Composer *composer, AnalysisType *analysis,
      AnalysisParts &parts, CFGNode cn, int dLevel, std::string dLevelStr)
      : DFTransferVisitor(parts, cn, state, dfInfo), modified(false),
        dLevel(dLevel), dLevelStr(dLevelStr), defaultLat(defaultLat),
        composer(composer), analysis(analysis) {
    // if(dLevel()>=1) dbg << "transfer A
    // prodLat="<<prodLat<<"="<<prodLat->str("    ")<<"\n";
    //  Make sure that all the lattices are initialized
    /*conVariableStateTransferst std::vector<Lattice*>& lattices =
    prodLat->getLattices(); for(std::vector<Lattice*>::const_iterator it =
    lattices.begin(); it!=lattices.end(); it++) (dynamic_cast<LatticeType
    *>(*it))->initialize();*/
    ...
    if (((ComposedAnalysis *)analysis)->useSSA)
      wildCardIndexPartEdge = NULLPartEdge;
    else
      wildCardIndexPartEdge =
          ((ComposedAnalysis *)analysis)->getDirection() == ComposedAnalysis::fw
  ...
  void visit(SgAssignOp *sgn) {
    ...
    {
      indent ind;
      dbg << (lhsLat ? lhsLat->str("") : "NULL") << "\n";
    }
    dbg << "rhsLat=\n";
    {
      indent ind;
      dbg << (rhsLat ? rhsLat->str("") : "NULL") << "\n";
    }
#endif
    SIGHT_VERB_FI()

    // Copy the lattice of the right-hand-side to both the left-hand-side
    // variable and to the assignment expression itself We only need to copy
    // rhsLat once since it is a fresh object greated by prodLat->get()
  ...
  void visit(SgAggregateInitializer *sgn) {
    ...
    if (inits.size() > 0) {
      LatticePtr initsCopy(
          dynamic_cast<LatticeType *>(getLattice(inits[0])->copy()));
      // res->copy(getLattice(inits[0]).get());
      modified = true;
      for (size_t i = 1; i < inits.size(); ++i)
        updateModified(
            initsCopy->meetUpdate(getLatticeOperand(sgn, inits[i]).get()));
  ...
  void visit(SgInitializedName *initName) {
    ...
    if (initName->get_initializer()) {
      ...
      SIGHT_VERB(dbg << "initializer exists: " << initLat->str("    ") << "\n",
                 1, dLevel)
      // If there was no initializer, var's lattice is set to the default
      // lattice
    } else {
  ...
  /*  void visit(SgVariableDeclaration *decl)
    {
      ...
      for(SgInitializedNamePtrList::iterator it=decl->get_variables().begin();
    it!=decl->get_variables().end(); it++) visit(*it);
    }*/
  ...
  // GB 2013-03-12: So we actually need this? It just gets a lattice at p and
  // then sets it to itself!
  void visit(SgPntrArrRefExp *sgn) {
    SIGHT_VERB(dbg << "<b>VariableStrateTransfer::visit(SgPntrArrRefExp *sgn)"
                   << endl,
               1, dLevel)
    // Copy data from the memory location identified by the array index
    // expression to the expression object of the SgPntrArrRefExp.
    // MemLocObjectPtrPair p = composer->Expr2MemLoc(sgn, part->inEdgeFromAny(),
    // analysis);
    // MemLocObjectPtr p = composer->Expr2MemLoc(sgn, part->inEdgeFromAny(),
    // analysis);
    ...
    // if(isSgPntrArrRefExp (sgn) &&
    //    (!isSgPntrArrRefExp (sgn->get_parent()) || !isSgPntrArrRefExp
    //    (isSgPntrArrRefExp (sgn->get_parent())->get_lhs_operand())))
    // {
    //   assert(p.mem);
