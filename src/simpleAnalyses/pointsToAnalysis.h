 ...
 * AbstractObjectMap (ProductLattice) stores an AbstractObjectSet for each
 * MemLocObjectPtr author: sriram@cs.utah.edu
 */
...
public:
  ...
  bool setLatticeOperand(SgNode *sgn, SgExpression *operand,
                         AbstractObjectSetPtr lat);
...
public:
  ...
  ComposedAnalysisPtr copy() {
    return boost::make_shared<PointsToAnalysis>(useSSA);
  }
  ...
  // void copyAbstractObjectSet(const AbstractObjectSet& aos,
  // std::list<MemLocObjectPtr>& list);

  // get the pointsToSet from the given map
  // boost::shared_ptr<AbstractObjectSet> getPointsToSet(SgNode* sgn,
  // PartEdgePtr pedge, AbstractObjectMap *aom);

  // wrap the given set PointsToMLPtr
  // PointsToMLPtr Expr2PointsToMLPtr(SgNode* sgn, PartEdgePtr pedge,
  // boost::shared_ptr<AbstractObjectSet> aom);
};
...
//! AbstractObjectSet is used to store the set of MemLocObjects denoted by an
//! expression.
...
public:
  PTMemLocObject(PartEdgePtr pedge, Composer *composer,
                 PointsToAnalysis *ptanalysis);
  ...
  virtual bool mayEqual(MemLocObjectPtr ml_p, PartEdgePtr pedge, Composer *comp,
                        ComposedAnalysis *analysis);
  virtual bool mustEqual(MemLocObjectPtr ml_p, PartEdgePtr pedge,
                         Composer *comp, ComposedAnalysis *analysis);
  virtual bool equalSet(MemLocObjectPtr ml_p, PartEdgePtr pedge, Composer *comp,
                        ComposedAnalysis *analysis);
  virtual bool subSet(MemLocObjectPtr ml_p, PartEdgePtr pedge, Composer *comp,
                      ComposedAnalysis *analysis);
  virtual bool isLive(PartEdgePtr pedge, Composer *comp,
                      ComposedAnalysis *analysis);
  virtual bool meetUpdate(MemLocObjectPtr ml_p, PartEdgePtr pedge,
                          Composer *comp, ComposedAnalysis *analysis);
