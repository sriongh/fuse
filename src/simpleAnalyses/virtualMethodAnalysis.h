 ...
 * hierarchy information as computed by
 * rose/src/midend/CallGraphAnalysis/ClassHierarchyGraph.*. This is used to
 * implement an ATS that wraps the input ATS but is missing edges along virtual
 * method calls that are inconsistent with the type of the object on which
 * they're called.
 *
...
// A tree that records the inheritance hierarchy of a given SgClassType, where
// the nodes are SgClassDefinitions and edges are inheritance relationships from
// each parent class to the derived class. This tree is focused on the
// inheritance information of a single SgClassDefinition or the minimal prefix
// common to multiple SgClassDefinitions. This is unlike the
// ClassHierarchyGraph, which contains information on all SgClassDefinitions.
class ClassInheritanceTree : public FiniteLattice {
  ...
  // A node in the inheritance tree. Each node encapsulates a single
  // SgClassDefinition and contains pointers to multiple parent nodes and a
  // single child node. Nodes are immutable. Once created and initialized they
  // must not be modified. This makes it possible for multiple trees to share
  // Nodes, which is important for memory use since many trees will be very
  // similar to each other. Note that the copy constructor copies just the bases
  // and leaves lists and not the Nodes inside the tree.
  class Node;
  ...
  public:
    ...
    bool operator==(NodePtr that) {
      dbg << "Node::== def=" << def << "=" << SgNode2Str(def)
          << ", that->def=" << that->def << "=" << SgNode2Str(that->def)
          << ", equal=" << (def == that->def) << endl;
      return def == that->def;
    }
    bool operator!=(NodePtr that) {
    ...
    std::string str(std::string indent = "") const {
      std::ostringstream s;
      s << "[ClassInheritanceTree::Node:(this=" << this
        << ", #parents=" << parents.size() << ", child=" << child.get() << "="
        << (child ? child->def : NULL) << endl;
  ...
  // derived. It is empty if there is nothing common among these
  // SgClassDefinitions.
  ...
  // Maps each SgClassDefinition to its nodes in the tree. A given declaration
  // may appear multiple times in the tree since a given set of
  // SgClassDefinitions may inherit from it via multiple paths.
  ...
  typedef enum {
    full, // The common hierarchy among all possible SgClassDefinitions,
  ...
  // Records whether the classes denoted by the tree include leaves as well as
  // the classes that derive from them (true) or just the derived classes
  // (false)
  bool inclusive;
...
// Records the number of entry/exit points that have been discovered into/out of
// each function so far. This makes it possible to track any newly-discovered
// entry points and ensure that ATS edges are generated for the corresponding
// exit points.
...
public:
  ...
  static VirtualMethodPartPtr create(PartPtr base, ComposedAnalysis *analysis) {
    VirtualMethodPartPtr newPart(boost::shared_ptr<VirtualMethodPart>(
        new VirtualMethodPart(base, analysis)));
    newPart->init();
    return newPart;
  }
  static VirtualMethodPartPtr create(const VirtualMethodPart &that) {
    VirtualMethodPartPtr newPart(
        boost::shared_ptr<VirtualMethodPart>(new VirtualMethodPart(that)));
    newPart->init();
    return newPart;
  }
...
protected:
  ...
  void wrapEdges(std::list<PartEdgePtr> &cache_Edges,
...
public:
  static VirtualMethodPartEdgePtr create(PartEdgePtr base,
                                         ComposedAnalysis *analysis) {
    VirtualMethodPartEdgePtr newPartEdge(
        boost::shared_ptr<VirtualMethodPartEdge>(
            new VirtualMethodPartEdge(base, analysis)));
    newPartEdge->init();
    return newPartEdge;
  }
  static VirtualMethodPartEdgePtr create(const VirtualMethodPartEdge &that) {
    VirtualMethodPartEdgePtr newPartEdge(
        boost::shared_ptr<VirtualMethodPartEdge>(
            new VirtualMethodPartEdge(that)));
    newPartEdge->init();
    return newPartEdge;
  }
  static VirtualMethodPartEdge *createRaw(PartEdgePtr base,
                                          ComposedAnalysis *analysis) {
    VirtualMethodPartEdge *newPartEdge =
        new VirtualMethodPartEdge(base, analysis);
    // newPartEdge->init();
    return newPartEdge;
  }
  ...
  VirtualMethodPartEdge *copy() const {
    return new VirtualMethodPartEdge(*this);
  }
...
public:
  ...
  ComposedAnalysisPtr copy() {
    return boost::make_shared<VirtualMethodAnalysis>();
  }
  ...
  std::string str(std::string indent) const { return "VirtualMethodAnalysis"; }
...
class VirtualMethodAnalysisTransfer
    : public VariableStateTransfer<ClassInheritanceTree,
                                   VirtualMethodAnalysis> {
public:
  VirtualMethodAnalysisTransfer(
      AnalysisParts &parts, CFGNode cn, NodeState &state,
      std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo, Composer *composer,
