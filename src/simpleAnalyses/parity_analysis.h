...
// This is a forward dataflow analysis that implements a simple abstraction of
// values that consists of the universal set, a single constant value and an
// empty set. It maintains a map of memory locations to these value
// abstractions.
...
class CPValueKind : public printable,
                    public boost::enable_shared_from_this<CPValueKind> {
public:
  // The different kinds of ParityValueObjects
  typedef enum {
    uninitialized, // The value has not been initialized (analyses may set this
                   // value to anything they want)
    concrete,      // The exact value is known and can be described with an
                   // SgValueExp
    // rank,     // The exact value is not known but we know their relative
    // order
    offsetList, // The value denotes the offset of some class/struct member from
                // a pointer the member's host object. Since the compiler can
                // add arbitrary padding, such values are lists of concrete
                // values and members' ranks within their host objects.
  ...
  //    - if the two objects could not be merged and therefore that must be
  //    placed after
  ...
  // Returns whether this CPValueKind denotes a non-strict subset (the sets may
  // be equal) of the set denoted by the given CPValueKind
...
class ParityValueObject : public FiniteLattice, public ValueObject {
  ...
  // StxValueObjects are used to identify both raw values and the locations of
  // class fields within a class. Since there may be arbitrary amounts of
  // padding between such fields, we can only determine the order of their
  // offsets within the class object, not the actual concrete values of these
  // offsets. Since the application may use these offsets to find additional
  // memory regions (e.g. a.b.c, (&a.b)+1 or array[1].a[2].b[3]), we represent
  // such derived offsets as sequences of StxValueObjects, the sum of which
  // corresponds to the actual offset. Thus, a.b.c: region(a), rank(b in a) +
  // rank(c in b)
  // (&a.b)+1: region(a), rank(b in a) + concrete(1)
  // array[1].a[2].b[3]: region(array), concrete(1*sizeof(elt of array)) +
  // rank(a in array elt) +
  //                                    concrete(2*sizeof elt of array[1]) +
  //                                    rank(b in array[1] elt) +
  //                                    concrete(3*sizeof elt of array[1].a[2])
  // We maintain such sums of StxValueObjects as a linked list, with each object
  // in the chain pointing to the next one. However, not all combinations need
  // to be represented explicitly: concrete + concrete: concrete object that
  // denotes the sum of the arguments concrete + rank, rank + concrete:
  // maintained as a list rank + rank: unknown
  // * + unknown, unknown + *: a single unknown value object
  /*  // pointer(memregion, *1) + *2, *2 + pointer(memregion, *1):
    //          resulting object is pointer(memregion, *1+*2), with the +
...
public:
  ...
  // than its parent
  ParityValueObjectPtr shared_from_this() {
    return boost::static_pointer_cast<ParityValueObject>(
        ValueObject::shared_from_this());
  }

  // Access functions.
  CPValueKindPtr getKind() const;
  // Sets this object's kind to the given kind, returning true if this causes
  // the ParityValueObject to change
...
public:
  CPExactKind(boost::shared_ptr<SgValueExp> exp)
      : CPValueKind(concrete), exp(exp) {}
  ...
  template <class DoOpType> CPValueKindPtr doUnaryOp(DoOpType &doOp);

  // Applies the given operation functor to the expression in this ConcreteKind,
  // whoch is assumed to be an integral type and returns the resulting CPKind
  template <class DoOpType> CPValueKindPtr doUnaryIntegralOp(DoOpType &doOp);

  // Applies the given operation functor to the expression in this ConcreteKind,
  // which is assumed to a floating point type and returns the resulting CPKind
  template <class DoOpType> CPValueKindPtr doUnaryFloatOp(DoOpType &doOp);
  ...
  // Creates a CPExactKind from the given value. This function is overloaded
  // with different argument types and for each type it creates a CPExactKind
  ...
  // Applies the given binary operation functor to the expression in this and
  // that ConcreteKinds, assuming they're both integral types and returns the
  // resulting CPKind
  template <class DoOpType>
  CPValueKindPtr doBinaryOpIntInt(DoOpType &doOp, CPExactKindPtr that);

  // Applies the given binary operation functor to the expression in this and
  // that ConcreteKinds, assuming at least one is not an integral type and
  // returns the resulting CPKind
  template <class DoOpType>
  ...
  // Returns a copy of this CPExactKind
  CPValueKindPtr copyV() const {
    return boost::make_shared<CPExactKind>(getVal());
  }
...
class CPOffsetListKind : public CPValueKind {
  ...
  public:
    ...
    bool operator==(const intWrap &that) const {
...
public:
  // Represents the rank of a field within a class/struct (full offset is not
  // known)
  ...
  // Represents a known offset from the address of some memory region or an
...
public:
  CPOffsetListKind(const rank &r) : CPValueKind(offsetList) {
    if (type == NULL)
      type = SageBuilder::buildLongLongType();
    offsetL.push_back(r);
  }
  ...
  CPOffsetListKind(const std::list<intWrap> &offsetL)
      : CPValueKind(offsetList), offsetL(offsetL) {
    if (type == NULL)
      type = SageBuilder::buildLongLongType();
  }
  ...
  CPValueKindPtr copyV() const {
    return boost::make_shared<CPOffsetListKind>(offsetL);
  }
...
public:
  CPMemLocObject(bool isCPFull, bool isCPEmpty, SgNode *base, PartEdgePtr pedge,
                 ParityAnalysis *analysis)
      : Lattice(pedge), MemLocObject(base), FiniteLattice(pedge),
        analysis(analysis), isCPFull(isCPFull), isCPEmpty(isCPEmpty) {}

  CPMemLocObject(MemRegionObjectPtr region, ParityValueObjectPtr index,
                 SgNode *base, PartEdgePtr pedge, ParityAnalysis *analysis)
      : Lattice(pedge), MemLocObject(region, index, base), FiniteLattice(pedge),
        analysis(analysis), isCPFull(false), isCPEmpty(false) {}

  CPMemLocObject(const CPMemLocObject &that)
      : Lattice(that.getPartEdge()), MemLocObject(that),
        FiniteLattice(that.getPartEdge()), analysis(that.analysis),
  ...
  // MemLocObjectPtr to full. Return true if this causes the object to change
  // and false otherwise.
  ...
  // returns true if this causes this to change and false otherwise
  // The part of this object is to be used for AbstractObject comparisons.
  bool meetUpdate(CPMemLocObject *that, PartEdgePtr pedge, Composer *comp,
...
public:
  ...
  bool transfer(PartPtr part, PartPtr supersetPart, CFGNode cn,
                NodeState &state,
                std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo);

  boost::shared_ptr<DFTransferVisitor>
  getTransferVisitor(PartPtr part, PartPtr supersetPart, CFGNode cn,
                     NodeState &state,
                     std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo);
  ...
  // pretty print for the object
  std::string str(std::string indent = "") { return "ParityAnalysis"; }

  friend class ParityAnalysisTransfer;
}; // class ParityAnalysis

class ParityAnalysisTransfer
    : public VariableStateTransfer<ParityValueObject, ParityAnalysis> {
private:
...
public:
  ...
  ParityAnalysisTransfer(PartPtr part, PartPtr supersetPart, CFGNode cn,
                         NodeState &state,
                         std::map<PartEdgePtr, std::vector<Lattice *>> &dfInfo,
