...
void *atsGraphEnterHandler(properties::iterator props) {
  return new atsGraph(props);
}
void atsGraphExitHandler(void *obj) {
  atsGraph *g = static_cast<atsGraph *>(obj);
  delete g;
}
...
atsGraph::atsGraph(properties::iterator props)
    : graph(properties::next(props)) {
  dot << "digraph atsGraph {" << endl;
  dot << "  compound=true;" << endl;
  dot << "  rankdir=" << (properties::getInt(props, "dirAligned") ? "TD" : "DT")
      << ";" << endl;
...
void *atsGraph::enterATSSubGraph(properties::iterator props) {
  ...
  dot << indent << "  color="
      << (properties::getInt(props, "crossAnalysisBoundary") ? "red" : "black")
      << ";" << endl;
  ...
  if (props.exists("source"))
    dot << indent << "      { rank=source; " << props.get("source") << " }"
        << endl;
...
void *atsGraph::enterATSGraphEdge(properties::iterator props) {
  ...
  if (properties::getInt(props, "crossFunc"))
    weight = 1;
  /*
  // Create anchors for from and to to make sure that we update their anchorID
  to use the same
  ...
  dot << indent << "a" << properties::getInt(props, "from") << " -> a"
      << (properties::getInt(props, "to")) << " [style=\"" << style << "\", "
