...
// Class that makes it possible to generate string labels by using the <<
// syntax. Examples: txt() << "a=" << (5+2) << "!"
//           txt("a=") << (5+2) << "!"
// Since this class is meant to be used by client code, it is placed inside the
// easier-to-access sight namespace
namespace sight {
struct txt : std::string {
  ...
  template <typename T> txt &operator<<(T const &t) {
    _stream << t;
    assign(_stream.str());
    return *this;
  }
