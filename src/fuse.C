...
// Regex expressions for the composition command, defined globally so that they
// can be used inside main (where they're initialized) as well as inside
// output_nested_results()
sregex Fuse::composer, Fuse::lcComposer, Fuse::lpComposer, Fuse::tComposer,
    Fuse::analysis, Fuse::noAnalysis, Fuse::cpAnalysis, Fuse::ldAnalysis,
    Fuse::ccsAnalysis, Fuse::dpAnalysis, Fuse::ptAnalysis, Fuse::vmAnalysis,
    Fuse::spcpAnalysis, Fuse::spvnAnalysis, Fuse::ssacpAnalysis,
    Fuse::ssaldAnalysis, Fuse::ssaccsAnalysis, Fuse::ssadpAnalysis,
    Fuse::ssaptAnalysis, Fuse::ssavmAnalysis, Fuse::ssaspcpAnalysis,
    Fuse::ssaspvnAnalysis, Fuse::dynMonitor, Fuse::analysisList, Fuse::compSpec;

// Records whether we've executed the basic Fuse initialization that is common
// to all instances of Fuse objects
...
Fuse::Fuse(int argc, char **argv) {
  ...
  //  When compiling fuse ROSE_PREFIX is defined as
  //  -DROSE_PREFIX="\"${top_builddir}\"" which is top of the build tree If fuse
  //  fails to find fuseLayout set up this environment variable appropriately.
  setenv("SIGHT_LAYOUT_EXEC",
         (txt() << ROSE_PREFIX << "/projects/fuse/src/fuseLayout").c_str(), 1);
  // setenv("SIGHT_LAYOUT_EXEC",
  // (txt()<<ROSE_PREFIX<<"/bin//fuseLayout").c_str(), 1);
  SightInit(argc, argv);
  modularApp::setNamedMeasures(namedMeasures("time", new timeMeasure()));

  initRegex();

  // Initialize the data structures that manage labeling the AST with the
  // results of Fuse analyses
...
// Returns the map the IDs of variables about which we'll report analysis
// results
...
void Fuse::initRegex() {
  ...
  // composer = as_xpr(icase("loosechain")) | icase("lc") | icase("loosepar") |
  // icase("lp");

  noAnalysis = as_xpr(icase("noanalysis")) | icase("none");
  cpAnalysis = as_xpr(icase("constantpropagationanalysis")) |
               icase("constprop") | icase("cp");
  ldAnalysis =
      as_xpr(icase("livedeadmemanalysis")) | icase("livedead") | icase("ld");
  ccsAnalysis = as_xpr(icase("callctxsensanalysis")) | icase("callctxsens") |
                icase("ccs");
  dpAnalysis =
      as_xpr(icase("deadpathelimanalysis")) | icase("deadpath") | icase("dp");
  ptAnalysis =
      as_xpr(icase("pointstoanalysis")) | icase("pointsto") | icase("pt");
  vmAnalysis = as_xpr(icase("virtualmethodanalysis")) | icase("virtualmem") |
               icase("vm");
  spcpAnalysis = as_xpr(icase("sparseconstantpropagationanalysis")) |
                 icase("spconstprop") | icase("spcp");
  spvnAnalysis = as_xpr(icase("sparsevaluenumberinganalysis")) |
                 icase("spvalnum") | icase("spvn");
  ssacpAnalysis = as_xpr(icase("SSA:constantpropagationanalysis")) |
                  icase("SSA:constprop") | icase("SSA:cp");
  ssaldAnalysis = as_xpr(icase("SSA:livedeadmemanalysis")) |
                  icase("SSA:livedead") | icase("SSA:ld");
  ssaccsAnalysis = as_xpr(icase("SSA:callctxsensanalysis")) |
                   icase("SSA:callctxsens") | icase("SSA:ccs");
  ssadpAnalysis = as_xpr(icase("SSA:deadpathelimanalysis")) |
                  icase("SSA:deadpath") | icase("SSA:dp");
  ssaptAnalysis = as_xpr(icase("SSA:pointstoanalysis")) |
                  icase("SSA:pointsto") | icase("SSA:pt");
  ssavmAnalysis = as_xpr(icase("SSA:virtualmethodanalysis")) |
                  icase("SSA:virtualmem") | icase("SSA:vm");
  ssaspcpAnalysis = as_xpr(icase("SSA:sparseconstantpropagationanalysis")) |
                    icase("SSA:spconstprop") | icase("SSA:spcp");
  ssaspvnAnalysis = as_xpr(icase("SSA:sparsevaluenumberinganalysis")) |
                    icase("SSA:spvalnum") | icase("SSA:spvn");
  dynMonitor = as_xpr(icase("dynamicmonitor")) | icase("dynmon") | icase("dm");
  analysis =
      by_ref(noAnalysis) | by_ref(cpAnalysis) | by_ref(ldAnalysis) |
      by_ref(ccsAnalysis) | by_ref(dpAnalysis) | by_ref(ptAnalysis) |
      by_ref(vmAnalysis) | by_ref(spcpAnalysis) | by_ref(spvnAnalysis) |
      ...
      by_ref(ssaspcpAnalysis) | by_ref(ssaspvnAnalysis) | by_ref(dynMonitor);
  analysisList = '(' >> *_s >> (by_ref(analysis) | by_ref(compSpec)) >> *_s >>
                 (!('(' >> *_s >> +_w >> *_s >> ')')) >>
                 *(*_s >> "," >> *_s >> (by_ref(analysis) | by_ref(compSpec)) >>
                   *_s >> (!('(' >> *_s >> +_w >> *_s >> ')'))) >>
                 *_s >> ')';
...
SSAGraph *Fuse::GetSSAGraph() const { return rootComposer->GetSSAGraph(cdip); }

/***************************************
 ***** Fuse::output_nested_results *****
 ***************************************/
Fuse::output_nested_results::output_nested_results(
    int tabs, composerType &parentComposerType,
    list<ComposedAnalysis *> &parentSubAnalyses, Composer **parentComposer,
    checkDataflowInfoPass *cdip)
    : tabs_(tabs), parentComposerType(parentComposerType),
      parentSubAnalyses(parentSubAnalyses), parentComposer(parentComposer),
      cdip(cdip) {}
...
// fuseCmd is the string representation of the composition of analyses that
// should be executed. Returns 0 on success and a non-zero error code on
// failure.
int Fuse::run(SgProject *project, const std::string &fuseCmd) {
  ...
  if (regex_match(fuseCmd, what, compSpec)) {
    ...
    output_nested_results::composerType rootComposerType =
        output_nested_results::unknown;

    cdip = new checkDataflowInfoPass();

    output_nested_results ons(0, rootComposerType, mySubAnalyses, &rootComposer,
                              cdip);
    std::for_each(what.nested_results().begin(), what.nested_results().end(),
                  ons);
    ...
    // Create the FuseLabeler, which needs to know the ATS computed by the
    // rootComposer
    if (labeler)
      delete labeler;
    ...
    // cout << "rootComposer="<<rootComposer<<"
    // cdip->getNumErrors()="<<cdip->getNumErrors()<<endl;
    if (cdip) {
...
// Places annotations into the AST that report the constant information that the
// most recent Fuse analysis composition has computed. verbose: if true, the
// annotations are printed after being placed
void Fuse::placeConstantPropagationAnnotations(bool verbose) {
  assert(fuseGloballyInitialized);
  ValueASTAttribute::placeLabeler(rootComposer, cdip, *labeler);
  annotator->annotateAstAttributesAsCommentsBeforeStatements(getProject(),
                                                             "fuse_cp_below");
  if (verbose)
    printAttributes<ValueASTAttribute>(labeler, vIDMap, "fuse_cp_below");
}

// Places annotations into the AST that report the reaching use-definition
// information that the most recent Fuse analysis composition has computed.
...
void Fuse::placeUseDefAnnotations(bool verbose) {
  ...
  {
    SIGHT_VERB_DECL(scope, ("Annotating As Comments", scope::high), 2,
                    apiDebugLevel)
    annotator->annotateAstAttributesAsCommentsBeforeStatements(getProject(),
                                                               "fuse_usedef");
  }
...
// Returns the set of entry FuseCFGNodes of the Abstract Transition System
// implemented by the given composer
...
// Returns the set of exit FuseCFGNodes of the Abstract Transition System
// implemented by the given composer
...
// Given a CFGNode from the VirtualCFG, returns the set of FuseCFGNodes that
// represent the nodes in the Abstract Transition System implemented by the
// given composer that refine this CFGNode.
...
// Given a CFGEdge from the VirtualCFG, returns the set of FuseCFGEdges that
// represent the edges in the Abstract Transition System implemented by the
// given composer that refine this CFGEdge.
...
std::set<FuseCFGEdgePtr> Fuse::GetATSEdges(const CFGEdge &ce,
  ...
  for (std::set<PartEdgePtr>::iterator r = refined.begin(); r != refined.end();
       ++r)
...
public:
  FuseCfgToDotImpl(std::ostream &o) : exploredNodes(), nodesPrinted(), o(o) {}
...
ostream &Fuse::cfgToDot(ostream &o, string graphName,
                        const std::set<FuseCFGNodePtr> &start) {
  o << "digraph " << graphName << " {\n";
  FuseCfgToDotImpl<FuseCFGNodePtr, FuseCFGEdgePtr, false> impl(o);
  for (std::set<FuseCFGNodePtr>::const_iterator n = start.begin();
       n != start.end(); ++n)
    impl.processNodes(*n);
  o << "}\n";
  return o;
...
// dump the full dot graph of a virtual control flow graph starting from CFGNode
// (start)
...
// dump the full dot graph of a virtual control flow graph starting from
// FuseCFGNodePtr (start)
...
/*
//dump the full dot graph of a virtual control flow graph starting from SgNode
(start) void cfgToDotForDebugging(SgNode* start, const std::string& file_name)
{
  ROSE_ASSERT (start != NULL);
...
//dump the full dot graph of a virtual control flow graph starting from CFGNode
(start) void cfgToDotForDebugging(CFGNode start, const std::string& file_name)
{
  cfgToDotForDebugging(makePtr<FuseCFGNode>(start), file_name);
}

//dump the full dot graph of a virtual control flow graph starting from
FuseCFGNodePtr (start) void cfgToDotForDebugging(FuseCFGNodePtr start, const
...
Fuse::getReachingDefsAtPhiDef(SSAMemLocObjectPtr pd) const {
  return GetSSAGraph()->getReachingDefsAtPhiDef(pd);
}

// Returns the SSA uses for the given def
const std::set<SSAMemLocObjectPtr> &
Fuse::getUses(SSAMemLocObjectPtr def) const {
  return GetSSAGraph()->getUses(def);
}

// Returns the immediate SSA defs (concrete and phi defs) that directly define
// the given use
const std::set<SSAMemLocObjectPtr> &
Fuse::getDirectDefs(SSAMemLocObjectPtr use) const {
  return GetSSAGraph()->getDefs(use);
}

// Returns the concrete defs that transitively reach the given use
std::set<SSAMemLocObjectPtr>
Fuse::getTransitiveDefs(SSAMemLocObjectPtr use) const {
  return GetSSAGraph()->getTransitiveDefs(use);
}

// Given a Fuse MemLocObject and a VariableIdMapping, return the VariableId this
// MemLocObject denotes or the empty VariableId
VariableId Fuse::getVariableId(MemLocObjectPtr ml,
...
// By making sure that we're not continually creating new SgVarRefExps we enable
// Fuse's caching mechanisms to work more efficiently and avoid caching errors
// when the same pointer ends up pointing to SgVarRefExp for different
// SgVariableSymbols.
std::map<SgVariableSymbol *, SgVarRefExp *> Fuse::varSym2Ref;

// Returns a SgVarRefExp for the given SgVariableSymbol, ensuring that during
// the entire process lifetime no SgVarRefExp pointer can ever correspond to
// multiple symbols.
...
// Label FuseLabeler::registerLabel(LabelProperty lp, const
// std::set<FuseCFGNodePtr>& nodes) {
//   Label l = Labeler::registerLabel(lp);
//   label2Node[l].insert(nodes.begin(), nodes.end());
//   for(std::set<FuseCFGNodePtr>::const_iterator n=nodes.begin();
//   n!=nodes.end(); ++n)
...
//   SIGHT_VERB_DECL(scope, ("FuseLabeler::createLabels()", scope::high), 2,
//   apiDebugLevel) RoseAst ast(root); for(RoseAst::iterator
//   i=ast.begin();i!=ast.end();++i) {
...
// //          assert(num==3);
// //          std::set<FuseCFGNodePtr> nodesCall = f.GetATSNodes(CFGNode(*i,
// 2));
// //          if(nodes.size()>0)
// registerLabel(LabelProperty(*i,LabelProperty::LABEL_FUNCTIONCALL),
// nodesCall);
// //
// //          std::set<FuseCFGNodePtr> nodesReturn = f.GetATSNodes(CFGNode(*i,
// 3));
...
// //        assert(num==2);
// //        std::set<FuseCFGNodePtr> nodesEntry =
// f.GetATSNodes(CFGNode(isSgFunctionDefinition(*i)->get_declaration()->get_parameterList(),
// 0));
...
//     // Create a label for each CFGNode associated with this SgNode that also
//     appears
//     // in the Fuse CFG
//     if(isSgGlobal(sgn)) continue;
//     if(isSgStatement(sgn)) {
//       //if(isSgExprStatement(sgn)) sgn =
//       isSgExprStatement(sgn)->get_expression(); if(!isSgExprStatement(sgn))
//       continue;
//     }
//     //cout << "Labeled: "<<SgNode2Str(sgn)<<endl;
//     if(isSgExprStatement(sgn) || isSgExpression(sgn) ||
//     isSgInitializedName(sgn)) {
//       SgExpression* expr=NULL;
//       if(isSgExprStatement(sgn))   expr =
//       isSgExprStatement(sgn)->get_expression(); else if(isSgExpression(sgn))
//       expr = isSgExpression(sgn);

//       SIGHT_VERB_DECL(scope, (txt()<<"    node="<<SgNode2Str(sgn)<<":
//       cfgIndexForEnd="<<(sgn)->cfgIndexForEnd()), 2, apiDebugLevel)
//       //cout << "expr="<<SgNode2Str(expr)<<",
//       (expr?expr:sgn)->cfgIndexForEnd()="<<(expr?expr:sgn)->cfgIndexForEnd()<<endl;
//       for(unsigned int idx=0; idx<=(expr?expr:sgn)->cfgIndexForEnd(); ++idx)
//       {
//         std::set<FuseCFGNodePtr> nodes = f.GetATSNodes(CFGNode(expr?expr:sgn,
//         idx)); SIGHT_VERB(dbg << "        idx="<<idx<<"
//         #nodes="<<nodes.size()<<endl, 2, apiDebugLevel) if(nodes.size()>0) {
//           LabelProperty::LabelType lt;
//           if(expr) {
//                  if(isSgFunctionCallExp(expr)       && idx==2) lt =
//                  LabelProperty::LABEL_FUNCTIONCALL;
//             else if(isSgFunctionCallExp(expr)       && idx==3) lt =
//             LabelProperty::LABEL_FUNCTIONCALLRETURN; else
//             if(isSgFunctionParameterList(expr) && idx==0) lt =
//             LabelProperty::LABEL_FUNCTIONENTRY; else
//             if(isSgFunctionDefinition(expr)    && idx==3) lt =
//             LabelProperty::LABEL_FUNCTIONEXIT; else lt =
//             (LabelProperty::LabelType)(LabelProperty::LABEL_OTHER+idx);
...
//        if(prop.getLabelType() == LabelProperty::LABEL_FUNCTIONCALL) return
//        CFGNode(sgn, 2);
//   else if(prop.getLabelType() == LabelProperty::LABEL_FUNCTIONCALLRETURN)
//   return CFGNode(sgn, 3); else if(prop.getLabelType() ==
//   LabelProperty::LABEL_FUNCTIONENTRY)      return CFGNode(sgn,
//   prop.getLabelType()); else if(prop.getLabelType() ==
//   LabelProperty::LABEL_FUNCTIONEXIT)       return CFGNode(sgn,
//   prop.getLabelType()); else
//   if(LabelProperty::LABEL_OTHER<=prop.getLabelType() &&
//   prop.getLabelType()<LabelProperty::LABEL_FUNCTIONCALL) {
//     //scope s(txt() << "node: "<<SgNode2Str(prop.getNode())<<",
//     label="<<prop.getLabelType()<<"="<<(prop.getLabelType()-LabelProperty::LABEL_OTHER));
...
// ValueASTAttribute::getAllVarSymbolsInScope(SgScopeStatement *scope) {
...
// // Returns a list of all the variables declared in the scopes that contain
// this node std::list<SgVariableSymbol*>
// ValueASTAttribute::getAllVarSymbols(SgNode *n) {
//   n = n->get_parent();
...
//       for(list<SgVariableSymbol*>::iterator v=scopeVars.begin();
//       v!=scopeVars.end(); v++) {
//         allVars.push_back(*v);
//       }
//     }
...
// ValueASTAttribute::ValueASTAttribute(SgNode* n, Composer* composer,
// checkDataflowInfoPass* cdip, dirT dir, std::string label):
//       composer(composer), cdip(cdip), dir(dir), label(label) {
//   SIGHT_VERB_DECL(scope, (txt()<<"ValueASTAttribute("<<SgNode2Str(n)<<")"),
//   1, valAttrDebugLevel)
...
//   accept CFGNodes if(isSgInitializedName(n)) cn = CFGNode(n, 1); else
//   if(isSgBinaryOp(n))   cn = CFGNode(n, 2); else if(isSgUnaryOp(n)) {
//     if(isSgCastExp(n))       cn = CFGNode(n, 0);
//     else if(isSgAddressOfOp(n) || isSgPointerDerefExp(n) || isSgPlusPlusOp(n)
//     || isSgMinusMinusOp(n)) cn = CFGNode(n, 1); else                     cn =
//     CFGNode(n, 2);
//   }
...
//   // Collect the PartEdges (computed by the given composer) that refine the
//   incoming or
//   // outgoing edges of the given SgNode
//   //collectRefinedEdges(composer, refinedEdges, (dir==above? cn.inEdges():
//   cn.outEdges())); if(dir==above) collectIncomingRefinedEdges(composer,
//   refinedEdges, cn); else           collectOutgoingRefinedEdges(composer,
//   refinedEdges, cn);

//   allInScopeVars = getAllVarSymbols(n);
//   SIGHT_VERB_IF(1, valAttrDebugLevel)
//     scope s("allInScopeVars");
//     for(std::list<SgVariableSymbol *>::iterator v=allInScopeVars.begin();
//     v!=allInScopeVars.end(); ++v) {
//       dbg << SgNode2Str(*v)<<endl;
//     }
//   SIGHT_VERB_FI()
// }

// ValueASTAttribute::ValueASTAttribute(CFGNode cn, Composer* composer,
// checkDataflowInfoPass* cdip, dirT dir, std::string label):
//       composer(composer), cdip(cdip), dir(dir), label(label), cn(cn) {
//   SIGHT_VERB_DECL(scope, (txt()<<"ValueASTAttribute("<<CFGNode2Str(cn)<<")"),
//   1, valAttrDebugLevel)
...
// // Apply Expr2Value for the given expression to all the edges in refinedEdges
// and return
// // the union of the resulting ValueObjects
// ValueObjectPtr ValueASTAttribute::Expr2Val(SgExpression* expr) {
//   ValueObjectPtr val;

//   SIGHT_VERB(dbg << "Expr2Val("<<SgNode2Str(expr)<<")
//   #refinedEdges="<<refinedEdges.size()<<endl, 1, valAttrDebugLevel)
//   for(std::set<PartEdgePtr>::iterator r=refinedEdges.begin();
//   r!=refinedEdges.end(); r++) {
...
//   }
//   SIGHT_VERB(dbg << "Expr2Val returning val="<<val.get()<<endl, 1,
//   valAttrDebugLevel)
...
//   (txt()<<"isConstantInteger(ref="<<SgNode2Str(ref)<<")"), 1,
//   valAttrDebugLevel) ValueObjectPtr val = Expr2Val(ref); SIGHT_VERB(dbg <<
//   "isConstantInteger() val="<<val.get()<<endl, 1, valAttrDebugLevel) if(!val)
//   return false; SIGHT_VERB(dbg << "isConstantInteger()
//   val="<<val->str()<<endl, 1, valAttrDebugLevel) if(val->isConcrete() &&
//   isStrictIntegerType(val->getConcreteType())) {
...
//   SIGHT_VERB_DECL(scope,
//   (txt()<<"isConstantInteger(varID="<<SgNode2Str(Fuse::getVIDMap()->getSymbol(varId))<<")"),
//   1, valAttrDebugLevel) ValueObjectPtr val =
//   Expr2Val(buildVarRefExp(isSgVariableSymbol(Fuse::getVIDMap()->getSymbol(varId))));
//   SIGHT_VERB(dbg << "isConstantInteger() val="<<val.get()<<endl, 1,
//   valAttrDebugLevel) if(!val) return false; SIGHT_VERB(dbg <<
//   "isConstantInteger()
//   val(concrete="<<val->isConcrete()<<")="<<val->str()<<endl, 1,
//   valAttrDebugLevel) if(val->isConcrete() &&
//   isStrictIntegerType(val->getConcreteType())) {
//     std::set<boost::shared_ptr<SgValueExp> > cVals = val->getConcreteValue();
//     if(cVals.size()==1) return true;
//   }
...
// CPAstAttributeInterface::ConstantInteger
// ValueASTAttribute::getConstantInteger(SgVarRefExp* ref) {
//   SIGHT_VERB(dbg << "getConstantInteger("<<SgNode2Str(ref)<<")"<<endl, 1,
//   valAttrDebugLevel) ValueObjectPtr val = Expr2Val(ref); assert(val);
//   if(val->isConcrete() && isStrictIntegerType(val->getConcreteType())) {
//     SIGHT_VERB(dbg << "val="<<val->str()<<endl, 1, valAttrDebugLevel)
//     std::set<boost::shared_ptr<SgValueExp> > cVals = val->getConcreteValue();
//     if(cVals.size()==1) return
//     getIntegerConstantValue((*cVals.begin()).get());
//   }
//   ROSE_ASSERT(0);
// }

// CPAstAttributeInterface::ConstantInteger
// ValueASTAttribute::getConstantInteger(VariableId varId) {
//   ValueObjectPtr val =
//   Expr2Val(buildVarRefExp(isSgVariableSymbol(Fuse::getVIDMap()->getSymbol(varId))));
//   if(val->isConcrete() && isStrictIntegerType(val->getConcreteType())) {
//     std::set<boost::shared_ptr<SgValueExp> > cVals = val->getConcreteValue();
...
//     ValueASTAttribute
//     exprLabel(isSgExprStatement(cn.getNode())->get_expression(), composer,
//     cdip, dir, label); return exprLabel.toString();
//   } else if(isSgReturnStmt(cn.getNode())) {
//     ValueASTAttribute
//     exprLabel(isSgReturnStmt(cn.getNode())->get_expression(), composer, cdip,
//     dir, label); return exprLabel.toString();
//   } else if(isSgIfStmt(cn.getNode())) {
//     ValueASTAttribute exprLabel(isSgIfStmt(cn.getNode())->get_conditional(),
//     composer, cdip, dir, label); return exprLabel.toString();
//   } else if(isSgCastExp(cn.getNode())) {
//     ValueASTAttribute exprLabel(isSgCastExp(cn.getNode())->get_operand(),
//     composer, cdip, dir, label); return exprLabel.toString();
...
//     isSgBasicBlock(cn.getNode())->get_statements(); if(stmt.size()==0) return
//     "[]"; ValueASTAttribute exprLabel(*stmt.begin(), composer, cdip, dir,
//     label); return exprLabel.toString();
//   } else if(isSgFunctionDeclaration(cn.getNode()) ||
//   isSgFunctionDefinition(cn.getNode()) ||
//   isSgPragmaDeclaration(cn.getNode())) {
//     return "[]";
//   }
...
//   {
//   SIGHT_VERB_DECL(scope,
//   (txt()<<"ValueASTAttribute::toString("<<CFGNode2Str(cn)<<")"), 1,
//   valAttrDebugLevel) s << "["<<label<<" : "<<SgNode2Str(cn.getNode())<<": ";
//   //cout << CFGNode2Str(cn) << ": "<<endl;;
//   int numConstants=0;
//   for(list<SgVariableSymbol *>::iterator v=allInScopeVars.begin();
//   v!=allInScopeVars.end(); v++) {
//     if(v!=allInScopeVars.begin()) { s << ", "; }

//     //SgVarRefExp* ref = buildVarRefExp(*v);
//     SgVarRefExp* ref = Fuse::varSymbol2Ref(*v);
//     SIGHT_VERB(dbg << "    "<<ref->unparseToString() << " = ", 1,
//     valAttrDebugLevel) s << ref->unparseToString()<<"=";
...
//       getConstantInteger(ref)<<endl;; numConstants++;
//     } else {
...
// void ValueASTAttribute::place(Composer* composer, checkDataflowInfoPass*
// cdip) {
//   RoseAst ast(getProject());
//   for(RoseAst::iterator i=ast.begin(); i!=ast.end();++i) {
//     if(isSgExpression(*i)) {
//       (*i)->setAttribute("fuse_cp_above", new ValueASTAttribute(*i, composer,
//       cdip, above, "fuse_cp_above"));
//       (*i)->setAttribute("fuse_cp_below", new ValueASTAttribute(*i, composer,
//       cdip, below, "fuse_cp_below"));
//     }
//   }
// }

// void ValueASTAttribute::placeLabeler(Composer* composer,
// checkDataflowInfoPass* cdip, Labeler& labeler) {
//   for(Labeler::iterator i=labeler.begin();i!=labeler.end();++i) {
//     SgNode* node=labeler.getNode(*i);
//     ROSE_ASSERT(node);
//     node->setAttribute("fuse_cp_above", new ValueASTAttribute(node, composer,
//     cdip, above, "fuse_cp_above")); node->setAttribute("fuse_cp_below", new
//     ValueASTAttribute(node, composer, cdip, below, "fuse_cp_below"));
//   }
// }

// void ValueASTAttribute::placeLabeler(Composer* composer,
// checkDataflowInfoPass* cdip, FuseLabeler& labeler) {
//   for(Labeler::iterator i=labeler.begin();i!=labeler.end();++i) {
//     SgNode* node=labeler.getNode(*i);
//     ROSE_ASSERT(node);
//     //cout << "placeLabeler cn="<<CFGNode2Str(labeler.getCFGNode(*i))<<endl;
//     node->setAttribute("fuse_cp_above", new
//     ValueASTAttribute(labeler.getCFGNode(*i), composer, cdip, above,
//     "fuse_cp_above")); node->setAttribute("fuse_cp_below", new
//     ValueASTAttribute(labeler.getCFGNode(*i), composer, cdip, below,
//     "fuse_cp_below"));
//   }
// }
...
//       ValueASTAttribute* above =
//       (ValueASTAttribute*)stmt->get_expression()->getAttribute("fuse_cp_above");
//       cout << "    above="<<above<<endl;
//      // ValueASTAttribute* below =
//      stmt->get_expression()->getAttribute("fuse_cp_aft");

//       // Determine whether any of the variables in the current scope are
//       constants above
//       // this statement
...
//       while(cur) {
//         cout << "    cur="<<SgNode2Str(cur)<<",
//         isConstantInteger="<<above->isConstantInteger(Fuse::getVIDMap()->variableId(cur))<<endl;
//         if(above->isConstantInteger(Fuse::getVIDMap()->variableId(cur)))
//           cout << "Variable
//           "<<cur->unparseToString()<<"="<<above->getConstantInteger(Fuse::getVIDMap()->variableId(cur))<<"
//           at "<<SgNode2Str(stmt)<<endl;
...
//         ValueASTAttribute* above =
//         (ValueASTAttribute*)(ref->getAttribute("fuse_cp_below"));
//         //cout << "    above="<<above<<endl;
//         //cout << " isConstantInteger="<<above->isConstantInteger(ref)<<endl;
//         cout.flush(); if(above->isConstantInteger(ref))
//           cout << SgNode2Str(ref)<<":
//           Value="<<above->getConstantInteger(ref)<<endl;
//       }
//     }
//   }
// }
...
//   for(std::set<FuseCFGNodePtr>::iterator n=nodes.begin(); n!=nodes.end();
//   ++n) {
//     const std::set<SSAMemLocObjectPtr>& uses = (*n)->getUses();
//     //dbg << "n="<<(*n)->toString()<<", #uses="<<uses.size()<<endl;
//     for(std::set<SSAMemLocObjectPtr>::const_iterator u=uses.begin();
//     u!=uses.end(); ++u) {
//       indent ind;
//       /*dbg << "u="<<(*u)->str()<<endl;
...
//   //scope
//   s(txt()<<"FuseUDAttribute::definitionsOfVariable("<<var.toString()<<"))");
...
//       if((isSgInitializedName((*u)->getBase()) &&
//       f.getVIDMap()->variableId(isSgInitializedName((*u)->getBase()))==var)
//       ||
//          (isSgVarRefExp((*u)->getBase())       &&
//          f.getVIDMap()->variableId(isSgVarRefExp((*u)->getBase()))==var)) {

//         // Iterate over all the defs of this use and add their FuseCFGNodes
//         to reachingDefNodes std::set<SSAMemLocObjectPtr> defs =
//         f.getTransitiveDefs(*u);
//         //dbg << "#defs="<<defs.size()<<endl;
//         for(std::set<SSAMemLocObjectPtr>::iterator d=defs.begin();
//         d!=defs.end(); ++d) {
...
//   for(std::set<FuseCFGNodePtr>::iterator n=reachingDefNodes.begin();
//   n!=reachingDefNodes.end(); ++n) {
//     const std::set<Label>& labels = f.getLabeler()->getLabels(*n);
//     reachingDefLabels.insert(labels.begin(), labels.end());
// /*    for(Labeler::iterator i=f.getLabeler()->begin();
// i!=f.getLabeler()->end(); ++i) {
//       // If the FuseCFGNodes associated with the current label contain the
//       current reaching
...
//   for(Labeler::iterator i=f.getLabeler()->begin(); i!=f.getLabeler()->end();
//   ++i) {
//     //cout << "place
//     sgn="<<f.getLabeler()->getNode(*i)<<"="<<SgNode2Str(f.getLabeler()->getNode(*i))<<endl;
//     FuseUDAttribute* attr = new
//     FuseUDAttribute(f.getLabeler()->getCFGNode(*i), f);
...
//     if(SgVarRefExp* ref = isSgVarRefExp(*i)) {
//       FuseUDAttribute* ud =
//       (FuseUDAttribute*)(ref->getAttribute("fuse_usedef"));
...
//! Wraps a Part with an API that is almost identical to the CFGNode API except
//! that all functions that return CFGNodes and CFGEdges now return boost
//! pointers to FuseCFGNodes and FuseCFGEdges. This is to make it possible for
//! code that currently uses VirtualCFGs to easily transition to working on top
//! of Fuse Abstract Transition Systems. It was not possible to fully retain the
//! VirtualCFG API since it returns copies of CFGNode and CFGEdge objects,
//! making it impossible to create new classs that implements the same API and
//! derive from CFGNode and CFGEdge.
...
FuseCFGNode::FuseCFGNode(Fuse *fuseAnalysis, PartPtr part)
    : fuseAnalysis(fuseAnalysis), part(part) {
  std::set<CFGNode> cns = part->CFGNodes();

  // FuseCFGNodes are only compatible with ATSs where each part denotes exactly
  // one CFGNode
  assert(cns.size() == 1);
...
// Wraps all the Parts in the given set with FuseCFGNodes and returns the set of
// these Nodes
std::set<FuseCFGNodePtr>
FuseCFGNode::parts2FuseCFGNodes(const std::set<PartPtr> &parts,
                                Fuse *fuseAnalysis) {
  std::set<FuseCFGNodePtr> nodes;
...
std::string FuseCFGNode::toString() const { return cn.toString(); }
...
std::string FuseCFGNode::toStringFuse() const { return part->str(); }

//! ID to use for Dot, etc.
std::string FuseCFGNode::id() const { return cn.id(); }

//! The underlying AST node
SgNode *FuseCFGNode::getNode() const { return cn.getNode(); }

//! An identifying index within the AST node given by getNode()
unsigned int FuseCFGNode::getIndex() const { return cn.getIndex(); }
...
vector<FuseCFGEdgePtr> FuseCFGNode::outEdges() const {
  ...
  for (list<PartEdgePtr>::iterator e = partEdges.begin(); e != partEdges.end();
       ++e)
...
// given part Get the mapping from each argument of the function call at the
// given part to the corresponding parameters the argument defines
const std::list<std::pair<SSAMemLocObjectPtr, std::set<SSAMemLocObjectPtr>>> &
FuseCFGNode::getFunc2Params() const {
...
FuseCFGNode::getDefsUsesAtPhiNode() const {
...
//! Constructor
FuseCFGEdge::FuseCFGEdge(Fuse *fuseAnalysis, PartEdgePtr pedge)
    : fuseAnalysis(fuseAnalysis), pedge(pedge) {
...
// Wraps all the PartEdges in the given set with FuseCFGEdges and returns the
// set of these Edges
std::set<FuseCFGEdgePtr>
FuseCFGEdge::parts2FuseCFGEdges(const std::set<PartEdgePtr> &pedges,
                                Fuse *fuseAnalysis) {
  std::set<FuseCFGEdgePtr> edges;
  for (std::set<PartEdgePtr>::const_iterator p = pedges.begin();
       p != pedges.end(); ++p)
...
std::string FuseCFGEdge::toString() const {
  /*  cout << "FuseCFGEdge::toString() part="<<pedge->str()<<endl;
    cout << "                         ce="<<CFGNode2Str(ce.source())<<" == >
    "<<endl; cout << " "<<CFGNode2Str(ce.target())<<endl;*/
  //  return ce.toString(); }
...
std::string FuseCFGEdge::toStringFuse() const { return pedge->str(); }

//! ID to use for Dot, etc.
std::string FuseCFGEdge::id() const { return ce.id(); }
...
EdgeConditionKind FuseCFGEdge::condition() const {
  /*cout << "FuseCFGEdge::condition() ce="<<CFGNode2Str(ce.source())<<" == >
  "<<endl; cout << " "<<CFGNode2Str(ce.target())<<endl;*/
  return ce.condition();
}

//! The label of the case represented by an eckCaseLabel edge
SgExpression *FuseCFGEdge::caseLabel() const { return ce.caseLabel(); }
