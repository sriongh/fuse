...
PartEdgePtr Lattice::getPartEdge() const {
  ...
  //  Dbg:: << "Lattice::getPartEdge()
  //  this->latPEdge="<<this->latPEdge->str()<<endl;
...
AbstractionPtr Lattice::genUnion(boost::shared_ptr<MAOMap> maoMap) {
  ...
  public:
    ...
    void operator()(boost::shared_ptr<void> curVal) {
      ...
      if (!accumLat)
        accumLat = curLat->copySharedPtr();
      else
        accumLat->meetUpdate(curLat);
    }
  };
  AOMKindOp x(this);
  maoMap->apply(x);
...
AbstractionPtr Lattice::genIntersection(boost::shared_ptr<MAOMap> maoMap) {
  ...
  //  Use intersectUpdate on all the values in maoMap to create a single
  //  intersect lattice
  class AOMKindOp : public MAOMap::applyMapFunc {
    Lattice *parent;
...
bool BoolAndLattice::intersectUpdate(Lattice *that) {
  ...
  if (state == 1)
    newState = dynamic_cast<BoolAndLattice *>(that)->state;
  // The intersection of two different states is the empty state
  else if (state != dynamic_cast<BoolAndLattice *>(that)->state)
    newState = -1;
...
{
  int newState = (state > (dynamic_cast<BoolAndLattice*>(that)->state ? state :
(dynamic_cast<BoolAndLattice*>(that)->state); return new
...
bool BoolAndLattice::setMLValueToFull(MemLocObjectPtr ml) {
  // Since this lattice may contain information for the entire application
  // state, we set the entire lattice to full just in case
  return setToFull();
}

// Returns whether this lattice denotes the set of all possible execution
// prefixes.
bool BoolAndLattice::isFull() { return state == 1; }

// Returns whether this lattice denotes the empty set.
bool BoolAndLattice::isEmpty() { return state == -1; }
...
bool IntMaxLattice::intersectUpdate(Lattice *that) {
  ...
  // Intersection of Full and anything is anything
  if (state == infinity)
    newState = dynamic_cast<IntMaxLattice *>(that)->state;
  // The intersection of two different states is the empty state
  else if (state != dynamic_cast<IntMaxLattice *>(that)->state)
    newState = -infinity;
...
{
  int newState = (state > (dynamic_cast<IntMaxLattice*>(that)->state ? state :
(dynamic_cast<IntMaxLattice*>(that)->state); return new IntMaxLattice(newState);
}*/
...
// Returns whether this lattice denotes the set of all possible execution
// prefixes.
bool IntMaxLattice::isFull() { return state == infinity; }

// Returns whether this lattice denotes the empty set.
bool IntMaxLattice::isEmpty() { return state == -infinity; }
...
string IntMaxLattice::str(string indent) const {
  ...
  outs << indent
       << (state == infinity ? "infinity"
                             : (state == -infinity ? "-infinity" : stateStr));
  return outs.str();
}
...
void ProductLattice::init(const vector<Lattice *> &lattices) {
  ...
  for (std::vector<Lattice *>::const_iterator l = lattices.begin();
       l != lattices.end(); l++)
    if (!((*l)->finiteLattice()))
      isFinite = false;
}
...
// returns a copy of this lattice
Lattice *ProductLattice::copy() const { return new ProductLattice(*this); }
...
//    may not require any actual insertions.
// The function takes newPEdge, the edge that points to the part within which
// the values of ml2ml should be
//    interpreted. It corresponds to the code region(s) to which we are
//    remapping.
// remapML must return a freshly-allocated object.
Lattice *ProductLattice::remapML(const std::set<MLMapping> &ml2ml,
                                 PartEdgePtr newPEdge) {
  ProductLattice *pl = new ProductLattice(latPEdge);
  for (std::vector<Lattice *>::iterator l = lattices.begin();
       l != lattices.end(); l++)
...
bool ProductLattice::replaceML(Lattice *newL) {
  ...
  bool modified = false;
  for (std::vector<Lattice *>::iterator t = lattices.begin(),
                                        n = nl->lattices.begin();
       t != lattices.end(); t++, n++)
...
// Computes the meet of this and that and returns the result
bool ProductLattice::finiteLattice() { return isFinite; }
...
bool ProductLattice::isFull() {
  // Since we're not sure whether the contents of the product lattice are
  // exhaustive, we don't bother checking if they are individually full and
  // instead conservatively return false.
  return false;
}
// Returns whether this lattice denotes the empty set.
