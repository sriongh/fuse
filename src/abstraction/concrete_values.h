...
typedef boost::shared_ptr<ConcreteUninitializedKind>
    ConcreteUninitializedKindPtr;
...
// Base class for the different kinds of values we can track. Used to identify
// both raw values, the locations of class fields within an object and the
// result of various arithmetic operatons among them. There may be arbitrary
// amounts of padding between fields within an object, meaning that we can only
// determine the order of their offsets within the object but not the actual
// concrete values of these offsets. Our representation of constants of either
// type is based on four different instances of the base class
// ConcreteValueKind:
//   - ConcreteUninitializedKind denotes the empty set of values
//   - ConcreteConcreteKind: denotes a concrete constant value that is
//   representable as a SgValueExp
//   - ConcreteOffsetListKind: denotes a unique location within a MemRegion;
//   they are represented as lists
//       of integral constant offsets (in terms of bytes) or where padding makes
//       this unknown, rank values that denote the position of a given location
//       relative to others.
//   - ConcreteUnknownKind: denotes the set of all values
class ConcreteValueKind
    : public sight::printable,
      public boost::enable_shared_from_this<ConcreteValueKind> {
public:
  ...
  typedef enum {
    ...
    offsetList, // The value denotes the offset of some class/struct member from
                ...
                // add arbitrary padding, such values are lists of integral
                // offset values and members' ranks within their host objects.
  ...
  // A comparable sub-class that makes it possible to compare instances of
  // valueKind in a standard way and use them as elements in hierarchical keys
  // used for creating hierarchical datastructures of AbstractObjects.
  ...
  public:
    ...
    bool equal(const comparable &that_arg) const {
      // try {
      const comparableKind &that =
          dynamic_cast<const comparableKind &>(that_arg);
  ...
  // Methods that type-cast this kind into one of its sub-kinds. If the caller
  // knows the correct sub-kind by calling getKind(), they can be sure that the
  // returned value is non-NULL.
  ConcreteUninitializedKindPtr asUninitializedKind() {
    return boost::static_pointer_cast<ConcreteUninitializedKind>(
        shared_from_this());
  }
  ...
  ConcreteOffsetListKindPtr asOffsetListKind() {
    return boost::static_pointer_cast<ConcreteOffsetListKind>(
        shared_from_this());
  }
  ...
  virtual ConcreteValueKindPtr op(SgBinaryOp *op,
                                  ConcreteValueKindPtr that) = 0;
  ...
  // Returns whether this ConcreteValueKind denotes a non-strict subset (the
  // sets may be equal) of the set denoted by the given ConcreteValueKind
  virtual bool subSetAO(ConcreteValueKindPtr that) = 0;

  // Computes the meet of this and that and returns the resulting kind
  virtual std::pair<bool, ConcreteValueKindPtr>
  meetUpdateAO(ConcreteValueKindPtr that) = 0;

  // Computes the intersection of this and that and returns the resulting kind
  virtual std::pair<bool, ConcreteValueKindPtr>
  intersectUpdateAO(ConcreteValueKindPtr that) = 0;
...
// ConcreteValueObjects form a set hierarchy and thus implement the
// AbstractionHierarchy interface. The hierarchy is: empty key: unknown
// non-empty:
...
public:
  ...
  intersectUpdateAO(ConcreteValueKindPtr that);
  ...
  ConcreteValueKindPtr copyAOType() const {
  ...
  // Don't add anything else since the hier-level key already separates the
  // empty set from all the other sets
  {}
...
public:
  ConcreteExactKind(boost::shared_ptr<SgValueExp> exp)
      : ConcreteValueKind(exact), exp(exp) {}
  ...
  template <class DoOpType> ConcreteValueKindPtr doUnaryOp(DoOpType &doOp);
  ...
  // ConcreteKind
  template <class DoOpType> ConcreteValueKindPtr doUnaryFloatOp(DoOpType &doOp);
  ...
  // Creates a ConcreteExactKind from the given value. This function is
  // overloaded with different argument types and for each type it creates a
  ...
  // that ExactKinds, assuming they're both integral types and returns the
  // resulting ConcreteKind
  template <class DoOpType>
  ConcreteValueKindPtr doBinaryOpIntInt(DoOpType &doOp,
                                        ConcreteExactKindPtr that);

  // Applies the given binary operation functor to the expression in this and
  // that ExactKinds, assuming at least one is not an integral type and returns
  // the resulting ConcreteKind
  template <class DoOpType>
  ...
  // Returns whether the SgValueExps denoted by val1 < the value denoted by
  // val2. If unknown, return unknownVal.
  ...
  // Returns a copy of this ConcreteExactKind
  ConcreteValueKindPtr copyAOType() const {
    return boost::make_shared<ConcreteExactKind>(getVal());
  }

  std::string str(std::string indent = "") const;

  // Generic wrapper for comparing SgNode*'s that implements the comparable
  // interface
...
//   - where padding makes this unknown, rank values that denote the position of
//   a given location relative to others.
//
// Below are some examples of how locations within MemRegions are modeled by
...
//
// This information is maintained by ConcreteOffsetListKind as a linked list.
// However, some combinations can ve represented more concisely:
...
class ConcreteOffsetListKind : public ConcreteValueKind {
  ...
  public:
    ...
    bool less(const comparable &that_arg) const {
      // try{
      const intWrap &that = dynamic_cast<const intWrap &>(that_arg);
      return (t < that.t) || (t == that.t && v < that.v);
      //} catch (std::bad_cast bc) {
      //  ROSE_ASSERT(0);
      //}
    }
    std::string str(std::string indent = "") const {
      return sight::txt() << "[type="
                          << (t == rankT ? "rankT"
                                         : (t == offsetT ? "offsetT" : "NULL"))
                          << ", v=" << v << "]";
    }
  }; // class intWrap
...
public:
  ...
  // List that identifies the unique location of a given MemLoc within a
  // MemRegion. It is a list of offsets and ranks, the former specifying a
  // concrete number of offset bytes and the latter specifying an order among
  // indexes within MemRegions but without specifying a concrete number of
  // bytes.
...
public:
  ...
  ConcreteOffsetListKind(const std::list<intWrap> &offsetL)
      : ConcreteValueKind(offsetList), offsetL(offsetL) {
    if (type == NULL)
      type = SageBuilder::buildLongLongType();
  }
  ...
  ConcreteValueKindPtr copyAOType() const {
    return boost::make_shared<ConcreteOffsetListKind>(offsetL);
  }
  ...
  void addHierSubKey(const AbstractionHierarchy::hierKeyPtr &key) {
    ...
    for (std::list<intWrap>::iterator o = offsetL.begin(); o != offsetL.end();
         o++)
...
public:
  ...
  // than its parent
  ConcreteValueObjectPtr shared_from_this() {
    return boost::static_pointer_cast<ConcreteValueObject>(
