...
void FuseInit(int argc, char **argv) {
  ...
  setenv("SIGHT_LAYOUT_EXEC", (sight::txt() << FUSE_LAYOUT).c_str(), 1);

  std::string title = (sight::txt() << "Process "
                                    << " Debug Output")
                          .c_str();
...
int main(int argc, char *argv[]) {
  ...
  Rose_STL_Container<SgNode *> pragmas =
      NodeQuery::querySubTree(project, V_SgPragma);
  for (Rose_STL_Container<SgNode *>::iterator p = pragmas.begin();
       p != pragmas.end(); p++) {
  ...
  for (int count = 0; s != sanalyses.end(); ++s, count++) {
    if (ConstPropCountAnalysis *cpc =
            dynamic_cast<ConstPropCountAnalysis *>(*s)) {
