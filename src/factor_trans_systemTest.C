...
int main(int argc, char **argv) {
  ...
  string fuseCmd = "";
  Rose_STL_Container<string> dataflowoptions =
      CommandlineProcessing::generateOptionList(args, "-fuse:");
  // std::vector<std::string>  dataflowoptions =
  // project->get_originalCommandLineArgumentList();
  if (dataflowoptions.size() > 1) {
    cout << "ERROR: too many -dataflow options!";
    exit(-1);
  }
  // If the caller provided a description of the composition structure on the
  // command line, set fuseCmd to it
...

  {
    ComposedAnalysisPtr analysisToTest =
        boost::make_shared<ConstantPropagationAnalysis>();
    set<ComposedAnalysisPtr> precisionAnalyses;
    precisionAnalyses.insert(boost::make_shared<CallContextSensitivityAnalysis>(
        2, CallContextSensitivityAnalysis::callSite));
    ...
    ComposedAnalysisSelfTester test(analysisToTest, precisionAnalyses,
                                    stxAnalysis);
    test.testAnalysis();
    return 0;
  }
  ...
  ComposedAnalysisPtr stxAnalysis =
      boost::make_shared<FactorTransSystemAnalysis>(
          true, // firstAnalysis
          transitionFactors,
          // 2, // myCommonFactor
          5, // maxSteps,
  ...
  set<ComposedAnalysisPtr> precisionAnalyses;
  precisionAnalyses.insert(boost::make_shared<FactorTransSystemAnalysis>(
      false, // firstAnalysis
      transitionFactors,
      // 2, // myCommonFactor
      5, // maxSteps,
