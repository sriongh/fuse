...
Function::Function(string name) {
  ...
  Rose_STL_Container<SgNode *> functions = NodeQuery::querySubTree(
      SageInterface::getProject(), V_SgFunctionDeclaration);
  for (Rose_STL_Container<SgNode *>::const_iterator it = functions.begin();
    ...
    if (isSgFunctionDeclaration(*it)->get_name().getString() == name) {
      ...
      if(isSgFunctionDeclaration(*it)->get_definition())
      {
        ...
        // ensure that either def has not been set yet or that there is a unique
      definition if(def==NULL) def =
      isSgFunctionDeclaration(*it)->get_definition(); else assert(def ==
...
void Function::init(SgFunctionDeclaration *sample) {
  ...
  if (!decl || !decl->get_definition() ||
      decl->get_definition()->attributeExists("fuse:UnknownSideEffects")) {
    // Add annotations to the function's parameters to make it clear to the
    // syntactic analysis that the SgInitializedNames of these parameters do not
    // have symbols
    if (decl) {
      ...
      for (SgInitializedNamePtrList::iterator p = params->get_args().begin();
        ...
        (*p)->setAttribute("fuse:NoSymbolExistsAttribute",
                           new NoSymbolExistsAttribute());
    }
  }
  ...
  // save the current function's definition inside def
  // ensure that either def has not been set yet or that there is a unique
  definition if(def==NULL) def = isSgFunctionDeclaration(*it)->get_definition();
  else
...
Function::Function(const Function &that) {
  ...
  it!=that.decls.end(); it++) {       //printf("      declaration: 0x%x\n",
  *it); decls.insert(*it);
  }*/
  decl = that.decl;
}
...
bool Function::isKnown() const { return decl != NULL; }
...
bool Function::eq(const Function &that) const {
  ...
  == decls.end())); if(decls.find(*it) == decls.end()) return false;
  }
  ...
  // dbg << const_cast<Function*>(this)->str() << " <eq>
  // "<<const_cast<Function&>(that).str()<<" : "<<!(decl != that.decl)<<endl;
  if (decl != that.decl)
    return false;
...
bool Function::lessThan(const Function &that) const {
  // dbg << const_cast<Function*>(this)->str() << " <lessThan>
  // "<<const_cast<Function&>(that).str()<<" : "<<(decl < that.decl)<<endl;
...
// Returns the Function object that refers to the function that contains the
// given SgNode
Function Function::getEnclosingFunction(SgNode *n, bool includingSelf) {
  SgFunctionDefinition *def =
      SageInterface::getEnclosingFunctionDefinition(n, includingSelf);
  if (def)
    return Function(def);
  else {
    SgFunctionDeclaration *decl =
        SageInterface::getEnclosingFunctionDeclaration(n, includingSelf);
    assert(decl);
...
CGFunction::iterator::iterator() { finished = true; }
...
CGFunction::iterator::iterator(const CGFunction *const func, direction dir) {
  ...
  // dbg << "CGFunction::iterator::iterator(func="<<func->str()<<",
  // direction="<<(dir==fw?"fw":"bw")<<endl;
  //  Start itn at the first node in cgNodes and keep walking through the
  //  cgNodes until we find one that has edges
  ...
  if (itn != func->cgNodes.end()) {
    ...
    /*dbg << "edges="<<endl;
    indent(1,1);
    for(set<SgDirectedGraphEdge*>::iterator e=edges.begin(); e!=edges.end();
    e++) { SgGraphNode* target = (dir == fw ? (*e)->get_to() :
    (*e)->get_from()); if(target) { Function
    func(isSgFunctionDeclaration(target->get_SgNode())); dbg <<
    func.str()<<endl; } else dbg << "NULL"<<endl;
    }*/
  } else
    finished = true;
}

// Returns a reference to CGFunction that matches the current SgGraphNode that
// this iterator refers to, pulling the reference from the given set of
// CGFunctions
const CGFunction *
CGFunction::iterator::getTarget(std::set<CGFunction> &functions) {
  ...
  // dbg <<
  // "isSgFunctionDeclaration(target->get_SgNode())->get_file_info()->isCompilerGenerated()="<<isSgFunctionDeclaration(target->get_SgNode())->get_file_info()->isCompilerGenerated()<<endl;

  // GB 2013-03-13: Now including these functions in the CG. Not clear if there
  // will be any issues associated with this decision Compiler-generated
  // functions do not appear as nodes in the call graph
  // if(isSgFunctionDeclaration(target->get_SgNode())->get_file_info()->isCompilerGenerated())
  // return NULL;
  ...
  // dbg << "getTarget returning NULL since couldn't find target
  // function"<<endl;

  // If we can't find it, return NULL
  assert(!"Error finding the target function of a call graph edge when the "
          "target is not compiler generated!");
  return NULL;
}
...
void CGFunction::iterator::advanceToNextValidPoint() {
  // dbg << "Function::iterator::advanceToNextValidPoint()
  // #edges="<<edges.size()<<" last edge="<<(ite == edges.end())<<endl;
  //  Loop for as long as ite is not pointing to a valid node and hasn't reached
  ...
  while (!finished && itn != func->cgNodes.end()) {
    ...
    if (ite == edges.end()) {
      /*dbg << "Function::iterator::advanceToNextValidPoint() while(),
      last="<<(itn == func->cgNodes.end())<<endl; indent ind;*/
      // Advance to the next SgGraphNode in cgNodes and keep walking through the
      // cgNodes until we find one that has edges
      itn++;
      while (itn != func->cgNodes.end()) {
        if (dir == fw)
    ...
    // Else, if it is not the last edges
      ...
      /*{
        SgGraphNode* target = (dir == fw ? (*ite)->get_to() :
      (*ite)->get_from()); if(target) { Function
      func(isSgFunctionDeclaration(target->get_SgNode())); dbg << "    advancing
      within CG Node target="<<func.str()<<",
      visited="<<(visitedCGNodes.find(target) != visitedCGNodes.end())<<endl; }
      else dbg << "    advancing within CG Node target=NULL"<<",
      visited="<<(visitedCGNodes.find(target) != visitedCGNodes.end())<<endl;
      }*/
...
TraverseCallGraph::TraverseCallGraph(SgIncidenceDirectedGraph *graph) {
  ...
  for (set<SgGraphNode *>::iterator itn = nodes.begin(); itn != nodes.end();
    ...
    // gettimeofday(&ntv, &ntz); printf("0 time = %lf secs, elapsed = %lf
    // secs\n", (ntv.tv_sec + 1000000*ntv.tv_usec) - (stv.tv_sec +
    // 1000000*stv.tv_usec), (ntv.tv_sec + 1000000*ntv.tv_usec) - (itv.tv_sec +
    // 1000000*itv.tv_usec)); stv.tv_sec = ntv.tv_sec; stv.tv_usec =
    // ntv.tv_usec;
    CGFunction func(isSgFunctionDeclaration(n), graph);
    // gettimeofday(&ntv, &ntz); printf("1 time = %lf secs, elapsed = %lf
    // secs\n", (ntv.tv_sec + 1000000*ntv.tv_usec) - (stv.tv_sec +
    // 1000000*stv.tv_usec), (ntv.tv_sec + 1000000*ntv.tv_usec) - (itv.tv_sec +
    // 1000000*itv.tv_usec)); stv.tv_sec = ntv.tv_sec; stv.tv_usec =
    // ntv.tv_usec;
    ...
    // GB 2013-03-13: Now including these functions in the CG. Not clear if
    // there will be any issues associated with this decision if this function
    ...
    // if(!func.get_file_info()->isCompilerGenerated())
    {
      ...
      func.get_name().str()); printf("TraverseCallGraph::TraverseCallGraph
      functions.find(func) == functions.end() = %d\n", functions.find(func) ==
      functions.end());*/

      functions.insert(func);
    }

    // gettimeofday(&ntv, &ntz); printf("2 time = %lf secs, elapsed = %lf
    // secs\n", (ntv.tv_sec + 1000000*ntv.tv_usec) - (stv.tv_sec +
    // 1000000*stv.tv_usec), (ntv.tv_sec + 1000000*ntv.tv_usec) - (itv.tv_sec +
    // 1000000*itv.tv_usec)); stv.tv_sec = ntv.tv_sec; stv.tv_usec =
    // ntv.tv_usec;
  ...
  // (ntv.tv_sec + 1000000*ntv.tv_usec) - (stv.tv_sec + 1000000*stv.tv_usec),
  // (ntv.tv_sec + 1000000*ntv.tv_usec) - (itv.tv_sec + 1000000*itv.tv_usec));
  // stv.tv_sec = ntv.tv_sec; stv.tv_usec = ntv.tv_usec;

  // initially, put all the nodes into noPred
  // initialized numCallers
  /*for(map<SgFunctionDefinition*, set<SgGraphNode*> >::iterator it =
     decl2CFNode.begin(); it!=decl2CFNode.end(); it++)*/
  ...
  for (set<CGFunction>::iterator func = functions.begin();
    ...
    // gettimeofday(&ntv, &ntz); printf("C time = %lf secs, elapsed = %lf
    // secs\n", (ntv.tv_sec + 1000000*ntv.tv_usec) - (stv.tv_sec +
    // 1000000*stv.tv_usec), (ntv.tv_sec + 1000000*ntv.tv_usec) - (itv.tv_sec +
    // 1000000*itv.tv_usec)); stv.tv_sec = ntv.tv_sec; stv.tv_usec =
    // ntv.tv_usec;
