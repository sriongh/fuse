...
protected:
  ...
  // advances this CFGIterator in the given direction. Forwards if fwDir=true
  // and backwards if fwDir=false. if pushAllChildren=true, all of the current
...
public:
  ...
  // Returns an empty CFGIterator that can be compared to any other CFGIterator
  // to check if it has completed passing over its iteration space
  ...
  public:
    checkpoint(const std::list<CFGNode> &remainingNodes,
               const std::set<CFGNode> &visited);
...
class back_dataflowCFGIterator : /*public virtual BaseCFG::back_dataflow,*/
                                 public virtual dataflowCFGIterator {
public:
  back_dataflowCFGIterator(const CFGNode terminator_arg)
      : dataflowCFGIterator(terminator_arg) {}
