...
CFGIterator::CFGIterator() { initialized = false; }
...
// returns true if the given CFGNode  is in the remainingNodes list and false
// otherwise
...
// advances this CFGIterator in the given direction. Forwards if fwDir=true and
...
// It is assumed that for a given CFGIterator pushAllChildren either always =
// true or always = false.
void CFGIterator::advance(bool fwDir, bool pushAllChildren) {
  assert(initialized);
  /*scope s(txt()<<"CFGIterator::advance(fwDir="<<fwDir<<",
  pushAllChildren="<<pushAllChildren<<")
  #remainingNodes="<<remainingNodes.size()); dbg<<"  visited=\n";
  ...
  if (remainingNodes.size() > 0) {
    ...
    if (pushAllChildren) {
      ...
      for (vector<CFGEdge>::iterator it = nextE.begin(); it != nextE.end();
           it++) {
        CFGNode nextN = (fwDir ? it->target() : nextN = it->source());
        /*dbg << "    nextN="<<CFGNode2Str(nextN)<<endl;
        dbg << "      CFGIterator::advance
      ...
      // Since pushAllChildren always = true or = false, we only need to worry
      // about managing visited in the true case
    }
  }
}
...
bool CFGIterator::eq(const CFGIterator &other_it) const {
  ...
  // printf("CFGIterator::eq() remainingNodes.size()=%d
...
CFGIterator::checkpoint::checkpoint(const list<CFGNode> &remainingNodes,
                                    const set<CFGNode> &visited) {
...
void CFGIterator::restartFromChkpt(CFGIterator::checkpoint &chkpt) {
  ...
  // The CFGIterator must become initialized because it is only possible to take
  // a checkpoints of initialized CFGIterators
  initialized = true;
}
...
string CFGIterator::str(string indent) const {
  ...
  if (initialized) {
    ...
    outs << "&nbsp;&nbsp;&nbsp;&nbsp;remainingNodes(#" << remainingNodes.size()
         << ") = " << endl;
    for (list<CFGNode>::const_iterator it = remainingNodes.begin();
         it != remainingNodes.end(); it++) {
      outs << "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;["
           << it->getNode()->unparseToString() << " | "
           << it->getNode()->class_name() << "]" << endl;
    }

    outs << "&nbsp;&nbsp;&nbsp;&nbsp;visited(#" << visited.size() << ")  = \n";
    for (set<CFGNode>::iterator it = visited.begin(); it != visited.end();
         it++) {
      outs << "&nbsp;&nbsp;&nbsp;&nbsp;[" << it->getNode()->unparseToString()
           << " | " << it->getNode()->class_name() << "]" << endl;
    }
...
dataflowCFGIterator::dataflowCFGIterator(const CFGNode terminator_arg)
...
dataflowCFGIterator::dataflowCFGIterator(const CFGNode start,
                                         const CFGNode terminator_arg)
...
void dataflowCFGIterator::init(const CFGNode start_arg,
                               const CFGNode terminator_arg) {
...
void dataflowCFGIterator::add(const CFGNode next) {
  // If this dataflow CFGIterator is not initialized, initialize it now since it
...
dataflowCFGIterator::checkpoint::checkpoint(
    const CFGIterator::checkpoint &iChkpt, const CFGNode terminator)
    : iChkpt(iChkpt), terminator(terminator) {}

dataflowCFGIterator::checkpoint::checkpoint(
    const dataflowCFGIterator::checkpoint &that)
...
string dataflowCFGIterator::checkpoint::str(string indent) const {
  ...
  outs << indent << "&nbsp;&nbsp;&nbsp;&nbsp;CFGIterator = \n"
       << iChkpt.str(indent + "&nbsp;&nbsp;&nbsp;&nbsp;") << "\n";
  outs << indent << "&nbsp;&nbsp;&nbsp;&nbsp;terminator = ["
       << terminator.getNode()->unparseToString() << " | "
       << terminator.getNode()->class_name() << "]" << endl;
  return outs.str();
}
...
dataflowCFGIterator::checkpoint dataflowCFGIterator::getChkpt() {
  ...
  return chkpt;
  // return new
  // dataflowCFGIterator::checkpoint::checkpoint(CFGIterator::getChkpt(),
  // terminator);
}

// Loads this CFGIterator's state from the given checkpoint.
void dataflowCFGIterator::restartFromChkpt(
    dataflowCFGIterator::checkpoint &chkpt) {
...
******* BACK_DATAFLOW *******
*****************************/
back_dataflowCFGIterator::back_dataflowCFGIterator(const CFGNode end,
                                                   const CFGNode terminator_arg)
    : CFGIterator(end), dataflowCFGIterator(end, terminator_arg) {}

void back_dataflowCFGIterator::operator++(int) { advance(false, false); }
