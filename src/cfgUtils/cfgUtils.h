...
class comparable {
public:
  // Operations that derived classes must implement. Note that it is possible
  // for the argument of these operations to be not of the same type as the
  // derived object implementing them. This happens only with special objects
  // that can handle type incompatibilities and is thus not an error. As such,
  // implementations should cast that to the type of the implementing object and
  // allow any bad_cast exceptions to propagate to the calling methods. This ==
  // That
  ...
  bool operator==(const comparablePtr &that) const {
...
template <typename Type> std::set<Type> list2set(const std::list<Type> l) {
  std::set<Type> s;
